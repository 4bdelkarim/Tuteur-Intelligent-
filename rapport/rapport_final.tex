...
\begin{itemize}
  ...
  \item \textbf{Détection de changement de sujet non activée}~: la méthode est implémentée mais pas encore branchée dans le flux principal. Elle permettrait de réinitialiser automatiquement la mémoire lors d'un changement radical de sujet.

  \item \textbf{Format de citation encore instable}~: le LLM cite parfois le titre complet de la section au lieu du seul numéro (ex.~: «~[\S{}LSTM]~» au lieu de «~[\S{}10.1]~»). Un raffinement du \textit{prompt} est nécessaire pour garantir des citations compactes et uniformes.
