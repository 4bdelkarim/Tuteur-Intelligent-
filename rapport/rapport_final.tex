...
\begin{description}[leftmargin=3.5cm,labelwidth=3cm]
  ...
  \item[D2L]  \textit{Dive into Deep Learning} --- manuel de référence (d2l.ai) constituant le corpus principal
...
\begin{itemize}
  \item \textbf{M2 désactivé en streaming}~: la vérification post-génération nécessite la réponse complète et n'est pas exécutée en mode conversationnel. Seul le M1 protège activement.

  \item \textbf{Évaluation pédagogique limitée}~: les métriques utilisées (hit@k, MRR, faithfulness) mesurent la qualité du retrieval et de l'ancrage, mais pas la qualité pédagogique des réponses (clarté, progressivité, adaptation au niveau). L'évaluation multi-tour reste également à développer.

  \item \textbf{Questions \textit{near-domain} non testées}~: l'évaluation du refus utilise des questions volontairement hors-domaine. Un test plus exigeant utiliserait des questions de ML absentes du corpus, plus difficiles à discriminer.
\end{itemize}
...
\smallskip
{\footnotesize\textbf{Note sur les références :} dans les réponses réelles du tuteur, les sources sont citées par leur numéro de section du cours (ex: [\S{}10.1], [\S{}5.3]). Chaque section correspond à un passage du corpus indexé --- pages web (D2L, atcold/NYU-DLSP21) ou supports de cours PDF. Les intitulés explicites [D2L, \S{}X.Y --- Titre] ci-dessous sont ajoutés pour la lisibilité du rapport et indiquent la source lorsqu'elle est identifiable.}
...
\textbf{Réponse}~: La backpropagation (rétropropagation) est l'algorithme fondamental pour l'entraînement des réseaux de neurones profonds. Elle calcule le gradient de la fonction de perte par rapport à chaque paramètre du réseau en appliquant la règle de dérivation en chaîne du calcul différentiel, en propageant les erreurs de la couche de sortie vers la couche d'entrée [D2L, \S{}5.3 — Rétropropagation][D2L, \S{}5.4 — Calcul efficace].

Son importance est double. D'une part, elle rend le calcul des gradients efficace en évitant le calcul redondant des dérivées partielles~: chaque couche réutilise les gradients déjà calculés par les couches suivantes, ce qui réduit la complexité de $\mathcal{O}(n^2)$ à $\mathcal{O}(n)$ [D2L, \S{}5.4 — Complexité algorithmique]. D'autre part, elle permet d'entraîner des réseaux de profondeur arbitraire, ce qui serait impossible avec une simple perturbation aléatoire des poids [D2L, \S{}5.3 — Fondements].

En pratique, la backpropagation est intégrée à tous les frameworks modernes de deep learning (PyTorch, TensorFlow) via le mécanisme de différenciation automatique, qui construit le graphe de calcul et applique la backpropagation sans intervention manuelle du développeur [D2L, \S{}2.5 — Différentiation automatique].
\end{quote}
...
La fonction sigmoïde $\sigma(x) = \frac{1}{1+e^{-x}}$ sature pour les grandes valeurs positives (sortie $\to 1$) et négatives (sortie $\to 0$), ce qui conduit au problème de \textit{vanishing gradient}~: le gradient devient quasi nul, ralentissant voire bloquant l'apprentissage des couches profondes [D2L, \S{}5.1.2 — Sigmoïde et « vanishing gradient »].

La fonction tanh $\tanh(x) = \frac{e^x - e^{-x}}{e^x + e^{-x}}$ souffre du même problème de saturation aux extrêmes, bien que sa sortie centrée en zéro (moyenne $\approx 0$) facilite la convergence par rapport à la sigmoïde [D2L, \S{}5.1.2 — Tanh].

La fonction ReLU $f(x) = \max(0, x)$ résout partiellement le problème de saturation~: pour $x > 0$, le gradient est constant (égal à 1), ce qui accélère considérablement la convergence [D2L, \S{}5.1.3 — ReLU]. Cependant, elle introduit le problème des «~neurones morts~»~: pour $x < 0$, le gradient est nul et le neurone cesse définitivement d'apprendre si le biais devient trop négatif [D2L, \S{}5.1.3 — Neurones morts].
\end{quote}
...
- Chaque document est identifié par le numéro de sa section du cours entre crochets (ex: « [\S{}10.1 — LSTM] »).
- Pour CHAQUE affirmation, cite la section source avec son numéro, exemple : « La backpropagation [...] [\S{}5.3]. »
- Citations multiples : [\S{}10.1][\S{}5.3].
\end{verbatim}
