...
La seconde question est une question de suivi (*follow-up*) : \textit{« compare-la avec le CNN »}. Sans mémoire conversationnelle, le pronom \textit{« la »} serait inintelligible pour le système. Avec la mémoire active, le tuteur comprend que la référence désigne la LSTM du tour précédent.
...
\begin{enumerate}
  ...
  \item \textbf{Le score reranker baisse sur les questions de suivi} : 0,986 $\rightarrow$ 0,488. Cette dégradation est attendue pour les questions comparatives, qui mobilisent des passages plus dispersés dans le corpus. Le \textit{query processing} compense partiellement en générant des sous-questions plus ciblées.
