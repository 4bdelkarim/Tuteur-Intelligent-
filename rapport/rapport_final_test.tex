...
% --- Logos ---
% (Placer les fichiers logo_isima.png et logo_limos.png dans rapport/figures/)
{\color{bleuPrincipal}\Large\textbf{ISIMA}\par}
\vspace{0.2cm}
...
\textbf{Date de présentation}  & Juillet 2026 \\
\textbf{Durée du stage}        & Février -- Juillet 2026 (6 mois) \\
\bottomrule
\end{tabular}
...
en premier lieu, je remercie chaleureusement M.~Vincent~BARRA, Professeur à l'ISIMA et responsable de ce stage, pour sa disponibilité, ses conseils avisés et la confiance qu'il m'a accordée tout au long de ce projet. Ses orientations m'ont permis de structurer ce travail et d'en approfondir les dimensions techniques avec rigueur.

Je remercie également l'ensemble de l'équipe pédagogique de la filière F4 pour la qualité de la formation dispensée, qui m'a fourni les bases théoriques et pratiques nécessaires à la conduite de ce projet, en particulier dans les domaines du traitement automatique du langage naturel et de l'apprentissage profond.

Je tiens à remercier le LIMOS pour l'accès à l'infrastructure de calcul ayant permis la phase d'évaluation de ce projet, ainsi que la communauté \textit{open-source} dont les outils et modèles (Ollama, ChromaDB, LangChain, BGE-M3, Qwen) constituent le socle technique de ce travail.
...
\begin{description}[leftmargin=3.5cm,labelwidth=3cm]
  ...
  \item[MTEB] \textit{Massive Text Embedding Benchmark}
  ...
  \item[TER]  Travail d'Étude et de Recherche
  \item[UCA]  Université Clermont Auvergne
...
Le Laboratoire d'Informatique, de Modélisation et d'Optimisation des Systèmes (LIMOS, UMR~6158) est une unité mixte de recherche du CNRS et de l'Université Clermont Auvergne (UCA), située sur le campus des Cézeaux à Aubière. Créé en 1982, le LIMOS rassemble aujourd'hui plus de 250 membres (chercheurs, enseignants-chercheurs, doctorants et personnels administratifs) répartis en six équipes de recherche.

Le laboratoire structure ses activités autour de trois axes scientifiques principaux~:
\begin{itemize}
  \item \textbf{Systèmes Informatiques et Réseaux}~: optimisation des réseaux, systèmes distribués, cybersécurité~;
  \item \textbf{Modélisation, Optimisation et Aide à la Décision}~: recherche opérationnelle, optimisation combinatoire, logistique~;
  \item \textbf{Science des Données et Intelligence Artificielle}~: apprentissage automatique, fouille de données, traitement du langage naturel.
\end{itemize}

C'est au sein de ce troisième axe que s'inscrit le présent stage, sous la responsabilité de M.~Vincent~BARRA, Professeur à l'ISIMA et chercheur au LIMOS. Ses travaux portent sur l'apprentissage profond, l'analyse de données multimodales et les systèmes de traitement automatique du langage naturel.

Le LIMOS met à disposition de ses chercheurs et stagiaires une infrastructure de calcul mutualisée, dont un serveur équipé d'un GPU NVIDIA H100 NVL (95~Go de VRAM). C'est sur cette infrastructure que l'ensemble des expérimentations du projet a été conduit, en cohérence avec la politique de mutualisation des ressources promue par le laboratoire.
...
\begin{enumerate}
  \item \textbf{Déploiement local}~: l'intégralité du pipeline doit s'exécuter sur l'infrastructure locale du laboratoire, sans dépendance à des API externes payantes.
...
Le chapitre~\ref{sec:rse} présente les considérations de responsabilité sociétale et environnementale (pratiques RSE du LIMOS, impact environnemental du projet, bilan carbone des déplacements). Le chapitre~\ref{sec:organisation} décrit l'organisation du projet, incluant les diagrammes de Gantt prévisionnel et réalisé, ainsi que l'analyse des écarts. Le chapitre~\ref{sec:etat-art} présente l'état de l'art du domaine, structuré par phase du pipeline RAG. Le chapitre~\ref{sec:methodologie} détaille la réalisation technique, de l'extraction des données à la génération des réponses. Le chapitre~\ref{sec:evaluation} présente le protocole d'évaluation et les résultats obtenus. Enfin, la conclusion synthétise les contributions et identifie les perspectives d'évolution, notamment vers un tuteur conversationnel de seconde génération.
...
Le Laboratoire d'Informatique, de Modélisation et d'Optimisation des Systèmes (LIMOS, UMR~6158) est une unité mixte de recherche du CNRS et de l'Université Clermont Auvergne. Le laboratoire s'inscrit dans la démarche de développement durable du CNRS, déclinée dans son Plan de Transition Écologique adopté en 2021. Cette démarche se traduit par plusieurs axes concrets~: rationalisation de l'usage des infrastructures de calcul mutualisées (serveurs partagés du mésocentre), politique d'achat de matériel reconditionné pour les postes de travail, et encouragement au calcul éco-responsable via l'optimisation des algorithmes et la réduction des redondances de calcul.

Le projet s'est déroulé sur l'infrastructure mutualisée du laboratoire, contribuant ainsi à une utilisation plus efficace des ressources de calcul existantes plutôt qu'à l'acquisition de matériel dédié supplémentaire.

\subsection{Impact environnemental du projet}

Le choix d'un déploiement entièrement local limite l'empreinte environnementale du projet par rapport à une solution reposant sur des API cloud~: aucune sollicitation de datacenter distant, pas de transfert de données sur le réseau. Les modèles retenus (0,6B à 14B paramètres) sont dimensionnés pour fonctionner sur du matériel accessible, ce qui réduit la consommation énergétique par rapport à des modèles plus volumineux.

\subsubsection{Estimation de la consommation énergétique}

Les phases de calcul intensif du projet se répartissent comme suit~:

\begin{itemize}
  \item \textbf{Extraction et ingestion}~: exécutées une fois, durée cumulée d'environ 3\,h.
  \item \textbf{Génération du jeu de données d'évaluation}~: environ 1\,h.
  \item \textbf{Évaluation complète}~: environ 2\,h.
\end{itemize}

Pour un total d'environ \textbf{6\,h de calcul GPU}. Le mix électrique français émet environ 50--60\,g de CO\textsubscript{2}eq par kWh (ADEME 2025), ce qui représente un impact carbone négligeable pour l'ensemble du projet.

En comparaison, l'utilisation d'une API cloud (GPT-4o par exemple) aurait engendré des transferts réseau répétés et une consommation dans un datacenter dont le mix énergétique est moins favorable (moyenne mondiale~: $\sim$400\,g CO\textsubscript{2}eq/kWh). Le déploiement local présente donc un double avantage~: économique (coût d'API nul) et environnemental (pas de transfert réseau, mix électrique français).

\subsection{Bilan carbone des déplacements}
\label{sec:bilan-carbone}

Le bilan carbone des déplacements a été calculé à l'aide de l'outil officiel de l'ADEME (\url{https://impactco2.fr/outils/transport}), conformément aux recommandations ISIMA. Les déplacements liés au stage se décomposent en deux catégories~:

\paragraph{Trajets domicile--lieu de stage}

Le stage s'est déroulé au LIMOS, sur le campus des Cézeaux à Aubière. Le trajet domicile--campus est d'environ 5\,km, effectué quotidiennement en tramway (ligne A, station «~Campus~») ou à pied. Le Tableau~\ref{tab:carbone-deplacements} synthétise le calcul~:

\begin{table}[H]
\centering
\begin{tabularx}{\linewidth}{L{3.5cm} C{2cm} C{2cm} C{2.5cm} C{2.5cm}}
\toprule
\rowcolor{bleuPrincipal}
\textcolor{white}{\textbf{Mode}} &
\textcolor{white}{\textbf{Distance/jour}} &
\textcolor{white}{\textbf{Jours}} &
\textcolor{white}{\textbf{Facteur (gCO\textsubscript{2}eq/km)}} &
\textcolor{white}{\textbf{Total (kgCO\textsubscript{2}eq)}} \\
\midrule
Tramway        & 10\,km (A/R) & 80 jours  & 3,8  & 3,04 \\
Marche à pied  & 10\,km (A/R) & 10 jours  & 0    & 0    \\
\midrule
\textbf{Total} &             &           &      & \textbf{3,04} \\
\bottomrule
\end{tabularx}
\caption{Bilan carbone des déplacements domicile--LIMOS (source~: ADEME Impact CO\textsubscript{2}, \url{impactco2.fr}).}
\label{tab:carbone-deplacements}
\end{table}

Le bilan carbone total des déplacements s'élève à environ \textbf{3,0\,kg CO\textsubscript{2}eq} sur l'ensemble de la période de stage, soit l'équivalent d'environ 15\,km en voiture thermique. Ce résultat, très modeste, reflète le choix d'une mobilité douce (transports en commun et marche à pied), cohérent avec la politique de développement durable de l'UCA et du LIMOS.

\paragraph{Déplacements annexes}

Aucun déplacement professionnel supplémentaire (conférence, réunion extérieure) n'a été effectué dans le cadre de ce stage. L'ensemble du projet s'est déroulé sur le site du laboratoire, en télétravail partiel pour les phases de rédaction, limitant ainsi l'empreinte carbone globale.

\paragraph{Bilan carbone global}

Le bilan carbone total du stage se décompose comme suit~:
\begin{itemize}
  \item Déplacements domicile--LIMOS~: 3,04\,kg CO\textsubscript{2}eq.
\end{itemize}
Soit un total d'environ \textbf{3,0\,kg CO\textsubscript{2}eq} pour l'ensemble du stage, un impact carbone négligeable, représentant moins de 0,04\,\% de l'empreinte carbone annuelle moyenne d'un Français (environ 9\,t CO\textsubscript{2}eq/an).
...
Le projet s'est déroulé sur un semestre (février--juillet 2026) et s'est articulé en quatre phases principales~:

\begin{enumerate}
  \item \textbf{Phase 1 --- Recherche et conception (février--mars)}~: étude bibliographique de l'état de l'art, analyse des besoins, choix technologiques, conception de l'architecture modulaire.
  \item \textbf{Phase 2 --- Implémentation du pipeline de préparation (mars--avril)}~: extraction PDF (GLM-OCR), extraction web, normalisation, \textit{chunking} parent-enfant, \textit{embedding} et indexation.
  \item \textbf{Phase 3 --- Implémentation du pipeline d'exécution (mai--juin)}~: \textit{retrieval} hybride, \textit{reranking}, traitement de la requête, génération, interface CLI.
  \item \textbf{Phase 4 --- Évaluation et rédaction (juin--juillet)}~: constitution du \textit{golden dataset}, évaluation quantitative (Ragas), optimisation des performances, rédaction du rapport.
\end{enumerate}

\subsection{Diagramme de Gantt prévisionnel}
\label{sec:gantt-previsionnel}

Le diagramme de Gantt prévisionnel (Figure~\ref{fig:gantt-prev}) a été établi en début de stage (février 2026) sur la base des objectifs initiaux et des contraintes calendaires connues. Il reflète une estimation prudente des durées de chaque phase, avec une marge de deux semaines en fin de projet pour absorber les imprévus.
...
\begin{ganttchart}[
  ...
  title/.style={fill=bleuSecondaire, draw=bleuSecondaire}
]{2026-02-03}{2026-07-10}
\gantttitlecalendar{month=name} \\

\ganttgroup{Phase 1 : Recherche \& Conception}{2026-02-03}{2026-03-13} \\
\ganttbar{État de l'art}{2026-02-03}{2026-02-28} \\
\ganttbar{Analyse des besoins}{2026-02-24}{2026-03-13} \\

\ganttgroup{Phase 2 : Implémentation (préparation)}{2026-03-09}{2026-04-24} \\
\ganttbar{Extraction PDF}{2026-03-09}{2026-04-03} \\
\ganttbar{Extraction Web}{2026-03-16}{2026-04-03} \\
\ganttbar{Chunking + Embedding}{2026-04-06}{2026-04-24} \\

\ganttgroup{Phase 3 : Implémentation (exécution)}{2026-04-20}{2026-06-12} \\
\ganttbar{Retrieval hybride}{2026-04-20}{2026-05-15} \\
\ganttbar{Reranking + QP}{2026-05-11}{2026-05-29} \\
\ganttbar{Génération + CLI}{2026-05-25}{2026-06-12} \\

\ganttgroup{Phase 4 : Évaluation \& Rédaction}{2026-06-08}{2026-07-10} \\
\ganttbar{Golden dataset}{2026-06-08}{2026-06-19} \\
\ganttbar{Évaluation}{2026-06-15}{2026-06-26} \\
\ganttbar{Rédaction du rapport}{2026-06-22}{2026-07-10} \\

\ganttmilestone{Soutenance}{2026-07-10}
\end{ganttchart}
\caption{Diagramme de Gantt prévisionnel du stage, établi en février 2026.}
...
Le diagramme de Gantt ci-dessous (Figure~\ref{fig:gantt-reel}) synthétise la planification réelle du projet.
...
\begin{ganttchart}[
  ...
  milestone top shift=0.35,
  time slot format=isodate,
  title/.style={fill=bleuPrincipal, draw=bleuPrincipal}
]{2026-02-03}{2026-07-10}
\gantttitlecalendar{month=name} \\

\ganttgroup{Phase 1 : Recherche \& Conception}{2026-02-03}{2026-03-20} \\
\ganttbar{État de l'art}{2026-02-03}{2026-03-06} \\
\ganttbar{Analyse des besoins}{2026-03-02}{2026-03-20} \\

\ganttgroup{Phase 2 : Implémentation (préparation)}{2026-03-16}{2026-05-01} \\
\ganttbar{Extraction PDF (GLM-OCR)}{2026-03-16}{2026-04-17} \\
\ganttbar{Extraction Web}{2026-03-23}{2026-04-10} \\
\ganttbar{Chunking + Embedding}{2026-04-13}{2026-05-01} \\

\ganttgroup{Phase 3 : Implémentation (exécution)}{2026-04-27}{2026-06-19} \\
\ganttbar{Retrieval hybride}{2026-04-27}{2026-05-22} \\
\ganttbar{Reranking + Query Processing}{2026-05-18}{2026-06-05} \\
\ganttbar{Génération + CLI}{2026-06-01}{2026-06-19} \\

\ganttgroup{Phase 4 : Évaluation \& Rédaction}{2026-06-15}{2026-07-10} \\
\ganttbar{Golden dataset}{2026-06-15}{2026-06-26} \\
\ganttbar{Évaluation Ragas}{2026-06-22}{2026-07-03} \\
\ganttbar{Rédaction du rapport}{2026-06-29}{2026-07-10} \\

\ganttmilestone{Soutenance}{2026-07-10}
\end{ganttchart}
\caption{Diagramme de Gantt réalisé du stage.}
...
La comparaison des deux diagrammes de Gantt fait apparaître trois écarts principaux, dont les causes et conséquences sont analysées ci-dessous.

\begin{table}[H]
\centering
\begin{tabularx}{\linewidth}{L{3cm} L{2.5cm} L{2.5cm} L{4cm}}
\toprule
\rowcolor{bleuPrincipal}
\textcolor{white}{\textbf{Tâche}} &
\textcolor{white}{\textbf{Prévisionnel}} &
\textcolor{white}{\textbf{Réalisé}} &
\textcolor{white}{\textbf{Explication}} \\
\midrule
Extraction PDF   & 4 semaines  & 5 semaines  & Complexité imprévue des figures composites (schémas avec formules intégrées) \\
Golden dataset   & 2 semaines  & 2 semaines  & Conforme (léger décalage dans le semestre) \\
Rédaction        & 3 semaines  & 2 semaines  & Gain de temps : contenu technique déjà documenté dans les journaux de session \\
\bottomrule
\end{tabularx}
\caption{Analyse des écarts entre le planning prévisionnel et le réalisé.}
\label{tab:ecarts}
\end{table}

\paragraph{Écart 1 --- Extraction PDF (+1 semaine)}

L'extraction des documents PDF a nécessité une semaine supplémentaire par rapport au planning prévisionnel. La cause principale est la complexité du traitement des figures composites (schémas comportant à la fois des éléments graphiques et des formules mathématiques intégrées), qui a exigé un paramétrage plus fin du pipeline GLM-OCR + Qwen3-VL. Cet écart, bien qu'anticipable, n'a pas eu d'impact sur les phases suivantes grâce aux marges intégrées dans la planification initiale.

\paragraph{Écart 2 --- Rédaction du rapport ($-$1 semaine)}

La rédaction du rapport a été réalisée en deux semaines au lieu des trois prévues. Ce gain s'explique par la tenue régulière de journaux de session (\texttt{rapport/journal\_*.md}) tout au long du projet, documentant les décisions techniques, les résultats intermédiaires et les corrections de bugs au fil de l'eau. Ces notes ont servi de base à la rédaction finale, réduisant significativement le temps de mise en forme.

\paragraph{Bilan global}

Le projet a été livré dans les délais impartis (juillet 2026). Les écarts observés sont mineurs ($\pm 1$ semaine) et se compensent mutuellement~: le retard initial en extraction a été absorbé par une avance en rédaction, grâce à la documentation continue du projet. La marge de deux semaines intégrée au planning prévisionnel s'est avérée correctement dimensionnée.
...
\begin{itemize}
  ...
  \item \textbf{En aval de la recherche}~: réévaluation des résultats par un modèle de \textit{reranking}~\cite{nogueira2019bert}, compression du contexte avant transmission à la génération~\cite{jiang2023llmlingua,xu2024recomp}.
\end{itemize}
...
\paragraph{Compression du contexte}

La qualité d'une réponse ne croît pas nécessairement avec le volume de contexte fourni au modèle de langage~: un contexte surchargé peut au contraire diluer l'information pertinente~\cite{gao2024ragsurvey}. La compression du contexte vise à réduire la longueur de l'entrée tout en préservant l'information essentielle.

Deux familles de méthodes coexistent~\cite{pan2024llmlingua2}~: la compression \textit{hard}, qui opère au niveau lexical en sélectionnant ou supprimant des \textit{tokens} (\textbf{LLMLingua}~\cite{jiang2023llmlingua}, \textbf{LLMLingua-2}~\cite{pan2024llmlingua2}), et la compression \textit{soft}, qui encode le contexte en représentations denses plus compactes. Plus récemment, \textbf{RECOMP}~\cite{xu2024recomp} propose une approche hybride combinant extraction et résumé abstrait, avec un modèle entraîné par distillation pour sélectionner les passages les plus informatifs. Le \textit{framework} \textbf{SeleCom}~\cite{selecom2026}, présenté à WWW~2026, redéfinit le rôle de l'encodeur comme un sélecteur d'information conditionné par la requête, montrant qu'une sélection ciblée surpasse une compression globale du document.
...
Pour le \textit{retrieval}, la combinaison \textbf{BM25}~\cite{robertson2009bm25} et \textbf{recherche dense} (BGE-M3), fusionnée par \textbf{RRF}~\cite{cormack2009rrf} puis réévaluée par un \textbf{\textit{cross-encoder}}~\cite{nogueira2019bert,xiao2024bge}, correspond à l'architecture hybride en deux étapes validée par Strich~\textit{et~al.}~\cite{strich2026bm25} comme la plus performante. Le traitement de la requête (\textit{query processing}) complète la chaîne en amont~; la compression du contexte, bien que décrite dans l'état de l'art, n'est volontairement pas branchée dans ce POC (axe d'évolution identifié).
...
Pour les sites web~: (i)~extraction du texte et conversion en un format structuré exploitable, (ii)~extraction des blocs de code, (iii)~élimination du bruit non pédagogique.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../rapport/figures/pipeline_global.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../rapport/figures/flux_ingestion.png}
...
Une première recherche, optimisée pour parcourir rapidement l'ensemble du corpus, ne garantit pas nécessairement un ordre de pertinence fin parmi les résultats obtenus~: une étape de réévaluation, appliquée aux meilleurs candidats déjà identifiés, permet d'affiner ce classement. Enfin, les passages retenus remontent vers leur parent~: la stratégie parent-enfant restitue le contexte complet des sections pertinentes à la génération, la compression du contexte (non branchée dans ce POC) étant identifiée comme axe d'évolution.

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../rapport/figures/flux_retrieval_concept.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{../rapport/figures/chunking_parent_child.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../rapport/figures/indexation.png}
...
Enfin, les passages retenus après réévaluation remontent vers leurs parents~: la stratégie parent-enfant restitue le contexte complet des sections pertinentes à la génération, sans étape de compression dédiée (non branchée dans ce POC).

\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../rapport/figures/retrieval.png}
...
La compression du contexte, étudiée dans l'état de l'art, n'est volontairement pas branchée dans ce POC.
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../rapport/figures/pipeline.png}
...
L'évaluation combine des métriques de \textit{retrieval} et de refus, complétées par des intervalles de confiance bootstrap à 95\,\% (1000~réplications).
...
La détection de refus a fait l'objet d'une attention particulière. Initialement basée sur une égalité stricte avec la phrase de refus du pipeline, elle a été élargie pour reconnaître les refus \textit{naturels} du LLM (phrases courtes indiquant l'incapacité à répondre à partir des documents, sans contenu factuel). Cette correction s'est avérée cruciale~: le LLM refusait correctement dans ses propres mots, mais l'évaluateur ne le détectait pas.
...
\bottomrule
\end{tabularx}
\caption{Performances de \textit{retrieval} aux quatre étapes de l'ablation (30~questions \textit{answerable}). Tous les scores sont accompagnés d'intervalles de confiance bootstrap à 95\,\% (non reproduits ici par lisibilité~; cf.~Figure~\ref{fig:ablation}).}
...
\caption{Étude d'ablation~: évolution des métriques de \textit{retrieval} aux quatre étapes. Les barres d'erreur représentent les intervalles de confiance bootstrap à 95\,\%.}
\label{fig:ablation}
\end{figure}
...
\bottomrule
\end{tabularx}
\caption{Performances par catégorie pour le système complet (hybride + \textit{cross-encoder}). Intervalles de confiance bootstrap 95\,\% entre crochets.}
...
\toprule
\rowcolor{bleuPrincipal}
\textcolor{white}{\textbf{Métrique}} & \textcolor{white}{\textbf{Score}} & \textcolor{white}{\textbf{IC 95\%}} \\
\midrule
Fidélité (\textit{faithfulness})     & 0,887 & [0,820~--~0,945] \\
Précision du contexte (\textit{context precision}) & 0,840 & [0,775~--~0,898] \\
Exactitude factuelle (\textit{custom correctness}, juge 1--5) & 0,786 & [0,714~--~0,857] \\
\bottomrule
\end{tabularx}
...
Le Tableau~\ref{tab:refus} synthétise les performances de refus sur les 15~questions \textit{unanswerable} hors-domaine, après correction de la fonction de détection de refus (\texttt{is\_refusal()}).
...
\midrule
Refus corrects   & 15/15 & \textbf{15/15} \\
Faux refus       & 0     & \textbf{0} \\
Faux non-refus   & 0     & \textbf{0} \\
Precision        & 1,000 & \textbf{1,000} \\
Recall           & 1,000 & \textbf{1,000} \\
F1               & 1,000 & \textbf{1,000} \\
\bottomrule
\end{tabularx}
\caption{Performances de refus sur les 15~questions \textit{unanswerable} hors-domaine. La baseline et le système complet atteignent tous deux 100\,\% de refus corrects.}
\label{tab:refus}
\end{table}

Le système complet et la baseline atteignent tous deux \textbf{100\,\% d'exactitude de refus} sur les questions hors-domaine. Ce résultat montre que le \textit{system prompt} seul, qui impose un ancrage strict dans le contexte, suffit à faire refuser le LLM sur des questions de domaines totalement étrangers. Les mécanismes M1 (score \textit{cross-encoder}) et M2 (juge post-génération) apportent une garantie supplémentaire pour les cas limites et permettent une économie de calcul (pas d'appel LLM si le score M1 est trop bas).

\textbf{Correction de la détection de refus.} Initialement, la fonction \texttt{is\_refusal()} détectait uniquement l'égalité stricte avec la phrase de refus du pipeline. Or le LLM refuse souvent dans ses propres mots («\,Les documents fournis ne contiennent pas cette information\,»), sans reproduire la phrase exacte. L'élargissement de la détection aux refus naturels (phrases courtes sans contenu factuel indiquant l'incapacité à répondre) a permis de capturer ces refus, faisant passer la baseline de 0/15 à 15/15. Ce correctif illustre que le système refusait correctement depuis le début --- l'évaluateur ne le mesurait simplement pas.
...
\textbf{Le refus est largement résolu par le \textit{system prompt}~; les mécanismes explicites fournissent une garantie supplémentaire.} La baseline et le système complet atteignent tous deux 100\,\% de refus corrects sur les questions hors-domaine, démontrant que le \textit{prompt} d'ancrage strict est le mécanisme de refus le plus efficace. Le M1 (score \textit{cross-encoder}) apporte une économie de calcul (pas d'appel LLM si le score est trop bas), tandis que le M2 (juge post-génération) protège contre les cas où le LLM utiliserait sa mémoire paramétrique malgré le \textit{prompt}.
...
\end{table}
% =============================================================
%     CHAPITRE 7 : TUTEUR CONVERSATIONNEL (v2.1)
% =============================================================
%                       CONCLUSION
% =============================================================
\section{Conclusion}
...
Le pipeline réalisé couvre l'ensemble de la chaîne, de l'extraction multimodale des documents (PDF et web) à la génération de réponses pédagogiques ancrées dans le corpus, en passant par une ingestion structurée et un retrieval hybride performant. L'architecture modulaire adoptée permet de faire évoluer chaque composant indépendamment, en fonction des progrès de l'état de l'art.

\subsection{Contributions principales}

Les contributions de ce travail sont de trois ordres.

\begin{enumerate}
  \item \textbf{Ingénierie d'un pipeline RAG complet en environnement local}~: de l'extraction multimodale (GLM-OCR + Qwen3-VL pour les figures) à la génération contrainte (Qwen2.5:14B), en passant par une stratégie de chunking parent-enfant et une indexation vectorielle (ChromaDB). L'ensemble fonctionne sans appel à des services cloud.

  \item \textbf{Validation empirique de l'architecture RAG avancée}~: le pipeline intègre les trois mécanismes clés du RAG avancé --- query processing (reformulation et décomposition), retrieval hybride avec RRF pondéré, et reranking par cross-encoder. Les résultats d'ablation démontrent la contribution de chaque composant, le cross-encoder apportant le gain le plus significatif (+6,7 pts hit@4).

  \item \textbf{Mécanisme de refus robuste}~: la combinaison d'un refus pré-génération basé sur le score du reranker (M1, seuil calibré à 0,1119, accuracy 100\,\%) et d'une vérification post-génération par juge LLM (M2) garantit qu'aucune réponse hors-contexte n'est présentée à l'étudiant. Sur les 15 questions hors-domaine du dataset de test, le refus est systématique et correct (15/15, F1~=~1,000).
\end{enumerate}

\subsection{Limites actuelles}

Le système présente plusieurs limites qui constituent autant d'axes d'amélioration pour la suite du projet.

\begin{itemize}
  \item \textbf{Absence de conversation}~: chaque question est traitée indépendamment. L'étudiant ne peut pas poser de questions de suivi naturelles («~et comment ça se compare à X~?~») sans répéter le contexte. La migration vers un tuteur conversationnel avec mémoire est la prochaine étape prioritaire (cf.~Annexe~\ref{app:architecture-v2}).

  \item \textbf{Posture purement transmissive}~: le système répond directement aux questions, sans guidage socratique ni adaptation du niveau d'explication. Le prompt actuel exclut explicitement le dialogue pédagogique («~une réponse directe, pas un dialogue socratique~»).

  \item \textbf{Évaluation limitée au comportement Q\&A}~: les métriques utilisées (hit@k, MRR, nDCG, faithfulness) mesurent la qualité du retrieval et de l'ancrage, mais pas la qualité pédagogique des réponses (clarté, progressivité, adaptation au niveau).

  \item \textbf{Corpus figé}~: le corpus indexé est constitué une fois pour toutes lors de l'ingestion. L'ajout de nouveaux documents nécessite une ré-exécution complète du pipeline d'extraction et d'indexation.

  \item \textbf{Questions \textit{near-domain} non testées}~: les questions \textit{unanswerable} utilisées pour l'évaluation du refus sont volontairement hors-domaine (géographie, biologie). Un test plus exigeant utiliserait des questions de ML absentes du corpus --- plus difficiles à discriminer pour le \textit{cross-encoder} comme pour le LLM.

  \item \textbf{Dépendance à la qualité de l'OCR}~: la qualité de l'extraction PDF conditionne toute la chaîne en aval. Des erreurs d'OCR (formules mal transcrites, tableaux mal structurés) se propagent jusqu'à la génération.
\end{itemize}

\subsection{Perspectives}

Les résultats obtenus ouvrent plusieurs perspectives d'évolution.

\paragraph{Tuteur conversationnel v2 (court terme)}

La transformation du système Q\&A en un véritable tuteur conversationnel, décrite comme perspective dans le rapport initial, a été réalisée dans la version 2.1 du système (Chapitre~\ref{sec:tuteur-v2}). Les développements futurs s'articulent autour des axes suivants. L'architecture cible, détaillée dans le document \texttt{docs/architecture\_tuteur.md}, s'articule autour de trois axes~:

\begin{enumerate}
  \item \textbf{Mémoire de conversation}~: gestion d'un historique multi-tour avec compression automatique des anciens messages pour rester dans la fenêtre de contexte (24K tokens).
  \item \textbf{Pédagogie socratique}~: guidage par questions plutôt que réponses directes, avec escalade progressive d'indices lorsque l'étudiant est bloqué.
  \item \textbf{Adaptation au niveau}~: modélisation du niveau de l'étudiant (débutant/intermédiaire/avancé) et adaptation automatique des explications.
\end{enumerate}

Le temps de développement estimé est de 10 à 15 jours ouvrés pour une version~1 complète, incluant une API REST (FastAPI) et une interface web (Gradio).

\paragraph{Améliorations techniques (moyen terme)}

\begin{itemize}
  \item \textbf{Migration vers Qwen3}~: le modèle de génération Qwen2.5:14B pourrait être remplacé par Qwen3:14B, qui apporte une fenêtre de contexte élargie et un mode de raisonnement hybride (thinking/non-thinking), avec un gain attendu sur la qualité des réponses complexes.
  \item \textbf{Embedding de nouvelle génération}~: Qwen3-Embedding-0.6B (premier au classement MMTEB) pourrait remplacer BGE-M3 pour l'embedding, potentiellement avec un gain en rappel.
  \item \textbf{Indexation incrémentale}~: permettre l'ajout de nouveaux documents au corpus sans ré-indexation complète, pour un cours qui évolue dans le temps.
  \item \textbf{Évaluation pédagogique}~: compléter les métriques de retrieval par des métriques de qualité pédagogique (clarté, progressivité, adaptation), via un protocole de type LLM-as-a-judge ou des tests utilisateurs.
\end{itemize}

\paragraph{Ouverture scientifique}

Au-delà du cas d'usage pédagogique, l'architecture développée dans ce projet est générique et peut s'appliquer à tout domaine nécessitant une recherche documentaire précise avec ancrage strict~: veille scientifique, analyse de brevets, support technique, documentation juridique. La modularité du pipeline facilite l'adaptation à de nouveaux corpus et de nouveaux modèles, au fur et à mesure des progrès de l'état de l'art.
...
Lewis, P., Perez, E., Piktus, A., Petroni, F., Karpukhin, V., Goyal, N. \textit{et~al.} (2020).
Retrieval-Augmented Generation for Knowledge-Intensive NLP Tasks.
\textit{NeurIPS}, vol.~33, pp.~9459--9474. arXiv:2005.11401.
...
\textit{Findings of EMNLP~2021}, pp.~3784--3803.

\bibitem{singh2025agenticrag}
Singh, A., Ehtesham, A., Kumar, S., Khoei, T.~T. (2025).
Agentic Retrieval-Augmented Generation: A Survey on Agentic RAG.
arXiv:2501.09136.

\bibitem{trec2025rag}
Upadhyay, S., Thakur, N., Pradeep, R., Craswell, N., Campos, D., Lin, J. (2025).
Overview of the TREC~2025 Retrieval Augmented Generation (RAG) Track.
arXiv:2603.09891.

\bibitem{wang2024bestpractices}
Wang, X., Wang, Z., Gao, X., Zhang, F., Wu, Y. \textit{et~al.} (2024).
...
\bibitem{ma2023queryrewriting}
Ma, X., Gong, Y., He, P. \textit{et~al.} (2023).
...
\textit{Found. Trends Inf. Retr.}, vol.~3, no.~4, pp.~333--389.
...
Zhang, Y., Li, M., Long, D., Zhang, X., Lin, H. \textit{et~al.} (2025).
...
\textit{Proc. SIGIR~2009}, pp.~758--759. ACM.
...
\bibitem{xiao2024bge}
Xiao, S., Liu, Z., Zhang, P., Muennighoff, N. (2024).
C-Pack: Packaged Resources To Advance General Chinese Embedding.
arXiv:2309.07597.

\bibitem{strich2026bm25}
Strich, D. \textit{et~al.} (2026).
From BM25 to Corrective RAG: Benchmarking Retrieval Strategies for Text-and-Table Documents.
arXiv:2604.01733.

\bibitem{fensore2025liverag}
Fensore, C., Dhole, K., Ho, J., Agichtein, E. (2025).
...
Pan, Z., Wu, Q., Jiang, H. \textit{et~al.} (2024).
...
\textit{Proc. WWW~2026}. ACM.

\bibitem{es2024ragas}
Es, S., James, J., Espinosa-Anke, L., Schockaert, S. (2024).
RAGAs: Automated Evaluation of Retrieval Augmented Generation.
\textit{Proc. EACL~2024 (System Demonstrations)}, pp.~150--158. arXiv:2309.15217.
...
\textit{Proc. EACL~2023}, pp.~2014--2037.

\bibitem{enevoldsen2025mmteb}
Enevoldsen, K., Chung, I., Kerboua, I. \textit{et~al.} (2025).
MMTEB: Massive Multilingual Text Embedding Benchmark.
arXiv:2502.13595.

\bibitem{wang2024e5}
Wang, L., Yang, N., Huang, X. \textit{et~al.} (2024).
Text Embeddings by Weakly-Supervised Contrastive Pre-training (E5).
arXiv:2212.03533.

\bibitem{li2023gte}
Li, Z., Zhang, X., Zhang, Y. \textit{et~al.} (2023).
...
\bibitem{qwen2025qwen3vl}
Qwen Team (2025). Qwen3-VL Technical Report.

\bibitem{glmocr2026}
Duan, S. \textit{et~al.} (2026).
GLM-OCR Technical Report. Zhipu AI / Tsinghua University.
arXiv:2603.10910.

\bibitem{smith2024chunking}
Smith, B., Troynikov, A. (2024).
Evaluating Chunking Strategies for Retrieval.
Chroma Technical Report.

\bibitem{yepes2024chunking}
Yepes, A.~J. \textit{et~al.} (2024).
Financial Report Chunking for Effective Retrieval Augmented Generation.
arXiv:2402.05131.

\bibitem{langchain2023}
LangChain (2023). Text Splitters Documentation.
\url{https://python.langchain.com/docs/}

\bibitem{lu2025hichunk}
Lu, W. \textit{et~al.} (2025).
HiChunk: Evaluating and Enhancing Retrieval-Augmented Generation with Hierarchical Chunking.
arXiv:2509.11552.

\bibitem{reuter2026chunking}
Reuter, M. \textit{et~al.} (2026).
A Systematic Investigation of Document Chunking Strategies and Embedding Sensitivity.
arXiv:2603.06976.

\bibitem{hrag2026semeval}
H-RAG at SemEval-2026 Task~8: Hierarchical Parent-Child Retrieval for Multi-Turn RAG Conversations.
arXiv:2605.00631.

\bibitem{qu2025semantic}
Qu, R., Tu, R., Bao, F.~S. (2025).
Is Semantic Chunking Worth the Computational Cost?
\textit{Findings of NAACL~2025}, pp.~2155--2177.
...
\begin{description}[leftmargin=1cm,labelwidth=3cm]
  \item[ADEME Impact CO\textsubscript{2}] \url{https://impactco2.fr/outils/transport} --- Outil officiel de calcul d'impact carbone des transports.
  \item[BGE-M3 (HuggingFace)] \url{https://huggingface.co/BAAI/bge-m3} --- Modèle d'embedding multilingue BAAI.
  \item[BGE-Reranker-v2-m3] \url{https://huggingface.co/BAAI/bge-reranker-v2-m3} --- Modèle de reranking cross-encoder.
  \item[ChromaDB] \url{https://www.trychroma.com/} --- Base de données vectorielle open-source.
  \item[DeepEval] \url{https://docs.confident-ai.com/} --- Framework d'évaluation de systèmes LLM.
  \item[Dive into Deep Learning] \url{https://d2l.ai/} --- Référence pédagogique du corpus.
  \item[GLM-OCR] \url{https://github.com/THUDM/GLM-OCR} --- Modèle d'OCR multimodal.
  \item[HuggingFace] \url{https://huggingface.co/} --- Plateforme de modèles open-weight.
  \item[LIMOS] \url{https://limos.fr/} --- Laboratoire d'accueil du stage.
  \item[MTEB Leaderboard] \url{https://huggingface.co/spaces/mteb/leaderboard} --- Benchmark de référence pour les modèles d'embedding.
  \item[NYU-DLSP21] \url{https://atcold.github.io/NYU-DLSP21/} --- Cours de Deep Learning de la NYU (A. Canziani).
  \item[Ollama] \url{https://ollama.com/} --- Runtime local pour LLMs et embeddings.
  \item[Qwen (GitHub)] \url{https://github.com/QwenLM/Qwen} --- Famille de modèles Qwen (Alibaba).
  \item[Ragas] \url{https://docs.ragas.io/} --- Framework d'évaluation RAG.
  \item[Rank-BM25] \url{https://github.com/dorianbrown/rank_bm25} --- Implémentation Python de BM25.
\end{description}
...
\textbf{Réponse}~: La backpropagation (rétropropagation) est l'algorithme fondamental pour l'entraînement des réseaux de neurones profonds. Elle calcule le gradient de la fonction de perte par rapport à chaque paramètre du réseau en appliquant la règle de dérivation en chaîne du calcul différentiel, en propageant les erreurs de la couche de sortie vers la couche d'entrée [DOC~1][DOC~2].

Son importance est double. D'une part, elle rend le calcul des gradients efficace en évitant le calcul redondant des dérivées partielles~: chaque couche réutilise les gradients déjà calculés par les couches suivantes, ce qui réduit la complexité de $\mathcal{O}(n^2)$ à $\mathcal{O}(n)$ [DOC~2]. D'autre part, elle permet d'entraîner des réseaux de profondeur arbitraire, ce qui serait impossible avec une simple perturbation aléatoire des poids [DOC~1].

En pratique, la backpropagation est intégrée à tous les frameworks modernes de deep learning (PyTorch, TensorFlow) via le mécanisme de différenciation automatique, qui construit le graphe de calcul et applique la backpropagation sans intervention manuelle du développeur [DOC~8].
\end{quote}
...
La fonction sigmoïde $\sigma(x) = \frac{1}{1+e^{-x}}$ sature pour les grandes valeurs positives (sortie $\to 1$) et négatives (sortie $\to 0$), ce qui conduit au problème de \textit{vanishing gradient}~: le gradient devient quasi nul, ralentissant voire bloquant l'apprentissage des couches profondes [DOC~3].

La fonction tanh $\tanh(x) = \frac{e^x - e^{-x}}{e^x + e^{-x}}$ souffre du même problème de saturation aux extrêmes, bien que sa sortie centrée en zéro (moyenne $\approx 0$) facilite la convergence par rapport à la sigmoïde [DOC~3].

La fonction ReLU $f(x) = \max(0, x)$ résout partiellement le problème de saturation~: pour $x > 0$, le gradient est constant (égal à 1), ce qui accélère considérablement la convergence [DOC~4]. Cependant, elle introduit le problème des «~neurones morts~»~: pour $x < 0$, le gradient est nul et le neurone cesse définitivement d'apprendre si le biais devient trop négatif [DOC~7].
\end{quote}
...
- Chaque document est identifié par un tag [DOC 1], [DOC 2], etc.
- Pour CHAQUE affirmation, cite le(s) document(s) source entre crochets.
\end{verbatim}
...
\texttt{scripts101/generator.py} (fonction \texttt{verify\_answer()}).
...
\section{Architecture du tuteur conversationnel v2}
\label{app:architecture-v2}

Une évolution majeure du système est en cours de conception~: le passage d'un mode questions/réponses à un véritable tuteur conversationnel. Le document \texttt{docs/architecture\_tuteur.md} détaille l'architecture cible, qui s'articule autour de trois axes~:

\begin{enumerate}
  \item \textbf{Conversation avec mémoire}~: gestion d'un historique multi-tour, détection de changement de sujet, compression automatique des anciens messages pour rester dans la fenêtre de contexte.
  \item \textbf{Pédagogie socratique}~: remplacement du prompt «~réponse directe~» par un système de guidage par questions, avec escalade progressive d'indices (léger $\to$ fort $\to$ explication complète) lorsque l'étudiant est bloqué.
  \item \textbf{Explications adaptatives}~: adaptation du niveau de détail et de formalisme mathématique selon le niveau perçu de l'étudiant (débutant, intermédiaire, avancé).
\end{enumerate}

L'interface évoluera également, de la CLI actuelle vers une API REST (FastAPI) puis une interface web (Gradio) avec chat interactif, sélecteur de niveau et mode socratique/direct.

% =============================================================
%                 FICHE D'AUTO-ÉVALUATION
% =============================================================
\section{Fiche d'auto-évaluation}
\label{app:auto-evaluation}

{\small\textit{Cette section est à compléter avec la fiche officielle fournie par l'ISIMA pour les étudiants de 3\textsuperscript{ème} année (ZZ3). En 2026, cette partie concerne uniquement les ZZ3.}}

\vspace{1cm}

\begin{table}[H]
\centering
\begin{tabularx}{\linewidth}{L{5cm} L{7cm}}
\toprule
\rowcolor{bleuPrincipal}
\textcolor{white}{\textbf{Critère}} & \textcolor{white}{\textbf{Auto-évaluation}} \\
\midrule
Respect du cahier des charges & Tous les objectifs ont été atteints~: pipeline d'extraction, ingestion, retrieval hybride, génération ancrée, évaluation quantitative. \\
\midrule
Qualité technique du travail  & Architecture modulaire, code documenté, 24 commits Git. Performances records~: hit@4=0.967, refus 100\,\%. \\
\midrule
Gestion de projet             & Planning respecté malgré un retard d'une semaine en extraction, compensé par la documentation continue. \\
\midrule
Communication                 & Réunions régulières avec le responsable ISIMA, journaux de session documentant les décisions et résultats. \\
\midrule
Autonomie et initiative       & Veille bibliographique approfondie, diagnostic et correction du bug Query Processing, proposition d'architecture v2. \\
\bottomrule
\end{tabularx}
\caption{Fiche d'auto-évaluation synthétique du stage.}
\label{tab:auto-eval}
\end{table}
