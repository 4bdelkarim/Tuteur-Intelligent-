---
source_url: "https://atcold.github.io/NYU-DLSP20/en/week02/02-2"
title: "Deep Learning"
date: "2020-02-03"
extractor: "generic"
---
...
- 
Linear: $Y=W\cdot X$ \[\begin{aligned} \frac{dC}{dX} &= W^\top \cdot \frac{dC}{dY} \\ \frac{dC}{dW} &= \frac{dC}{dY} \cdot X^\top \end{aligned}\]
- 
ReLU: $y=(x)^+$ \[\frac{dC}{dX} = \begin{cases} 0 & x<0\\ \frac{dC}{dY} & \text{otherwise} \end{cases}\]
- 
...
- 
Can be split into $n$ branches similarly \[\frac{dC}{dX}=\frac{dC}{dY_1}+\frac{dC}{dY_2}\]
 
- 
- 
Add: $Y=X_1+X_2$ - 
With two variables being summed, when one is perturbed, the output will be perturbed by the same quantity, i.e. \[\frac{dC}{dX_1}=\frac{dC}{dY}\cdot1 \quad \text{and}\quad \frac{dC}{dX_2}=\frac{dC}{dY}\cdot1\]
 
- 
- 
Max: $Y=\max(X_1,X_2)$ - Since this function can also be represented as
 - 
Therefore, by the chain rule, \[\frac{dC}{dX_1}=\begin{cases} \frac{dC}{dY}\cdot1 & X_1 > X_2 \\ 0 & \text{else} \end{cases}\]
 

[LogSoftMax ](https://www.youtube.com/watch?v=d9vdh3b787Y&t=3953s)*vs.* SoftMax
...
\[\log\left(\frac{\exp(s)}{\exp(s) + 1}\right)= s - \log(1 + \exp(s))\] ![Plot of logarithmic part of the functions](/NYU-DLSP20/images/week02/02-2/02-2-3.png)[Practical tricks for backpropagation](https://www.youtube.com/watch?v=d9vdh3b787Y&t=4891s)
...
\[b_{[i,j]}^{'} = \frac{b_{[i,j]} - m_b}{\max(\sigma_b, \epsilon)}\]where $\epsilon$ is an arbitrarily small number that we use to avoid division by zero. Repeat the same for green and red channels. This is necessary to get a meaningful signal out of images taken in different lighting; for example, day lit pictures have a lot of red while underwater pictures have almost none.
...
\[L(S, w) = C(S, w) + \alpha \Vert w \Vert^2\\ \frac{\partial R}{\partial w_i} = 2w_i\\ w_i = w_i - \eta\frac{\partial L}{\partial w_i} = w_i - \eta \left( \frac{\partial C}{\partial w_i} + 2 \alpha w_i \right)\]To understand why this is called weight decay, note that we can rewrite the above formula to show that we multiply $w_i$ by a constant less than one during the update.

\[w_i = (1 - 2 \eta \alpha) w_i - \eta\frac{\partial C}{\partial w_i}\]L1 regularization (Lasso) is similar, except that we use $\sum_i \vert w_i\vert$ instead of $\Vert w \Vert^2$.
