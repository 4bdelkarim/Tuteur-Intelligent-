---
source_url: "https://atcold.github.io/NYU-DLSP20/en/week11/11-1"
title: "Deep Learning"
date: "2020-04-13"
extractor: "generic"
---
...
\[\text{ReLU}(x) = (x)^{+} = \max(0,x)\]
 ![](/NYU-DLSP20/images/week11/11-1/ReLU.png) **Fig. 1**: ReLU

### RReLU - `nn.RReLU()`

There are variations in ReLU. The Random ReLU (RReLU) is defined as follows.

\[\text{RReLU}(x) = \begin{cases} x, & \text{if} x \geq 0\\ ax, & \text{otherwise} \end{cases}\] ![](/NYU-DLSP20/images/week11/11-1/RRelU.png)**Fig. 2**: ReLU, Leaky ReLU/PReLU, RReLU
...
\[\text{LeakyReLU}(x) = \begin{cases}
    ...
    \end{cases}\]
 ![](/NYU-DLSP20/images/week11/11-1/LeakyReLU.png) **Fig. 3**: LeakyReLU
...
![](/NYU-DLSP20/images/week11/11-1/PReLU.png)**Fig. 4**: ReLU
...
\[\text{Softplus}(x) = \frac{1}{\beta} * \log(1 + \exp(\beta * x))\]
 ![](/NYU-DLSP20/images/week11/11-1/Softplus.png) **Fig. 5**: Softplus
...
\[\text{ELU}(x) = \max(0, x) + \min(0, \alpha * (\exp(x) - 1)\]
 ![](/NYU-DLSP20/images/week11/11-1/ELU.png) **Fig. 6**: ELU
...
\[\text{CELU}(x) = \max(0, x) + \min(0, \alpha * (\exp(x/\alpha) - 1)\]
 ![](/NYU-DLSP20/images/week11/11-1/CELU.png) **Fig. 7**: CELU

### SELU - `nn.SELU()`

\[\text{SELU}(x) = \text{scale} * (\max(0, x) + \min(0, \alpha * (\exp(x) - 1))\]
 ![](/NYU-DLSP20/images/week11/11-1/SELU.png) **Fig. 8**: SELU
...
![](/NYU-DLSP20/images/week11/11-1/GELU.png)**Fig. 9**: GELU

### ReLU6 - `nn.ReLU6()`

\[\text{ReLU6}(x) = \min(\max(0,x),6)\]
 ![](/NYU-DLSP20/images/week11/11-1/ReLU6.png) **Fig. 10**: ReLU6
...
\[\text{Sigmoid}(x) = \sigma(x) = \frac{1}{1 + \exp(-x)}\]
 ![](/NYU-DLSP20/images/week11/11-1/Sigmoid.png) **Fig. 11**: Sigmoid
...
\[\text{Tanh}(x) = \tanh(x) = \frac{\exp(x) - \exp(-x)}{\exp(x) + \exp(-x)}\]
 ![](/NYU-DLSP20/images/week11/11-1/Tanh.png) **Fig. 12**: Tanh
...
\[\text{SoftSign}(x) = \frac{x}{1 + |x|}\]
 ![](/NYU-DLSP20/images/week11/11-1/Softsign.png) **Fig. 13**: Softsign
...
\[\text{Tanhshrink}(x) = x - \tanh(x)\]
 ![](/NYU-DLSP20/images/week11/11-1/Tanhshrink.png) **Fig. 15**: Tanhshrink
...
\[\text{SoftShrinkage}(x) = \begin{cases}
      ...
      0, & \text{otherwise}
    \end{cases}\]
 ![](/NYU-DLSP20/images/week11/11-1/Softshrink.png) **Fig. 16**: Softshrink
...
\[\text{HardShrinkage}(x) = \begin{cases}
      ...
      0, & \text{otherwise}
    \end{cases}\]
 ![](/NYU-DLSP20/images/week11/11-1/Hardshrink.png) **Fig. 17**: Hardshrink
...
\[\text{LogSigmoid}(x) = \log\left(\frac{1}{1 + \exp(-x)}\right)\]
 ![](/NYU-DLSP20/images/week11/11-1/LogSigmoid.png) **Fig. 18**: LogSigmoid
...
\[z_i = \begin{cases}0.5(x_i-y_i)^2, \quad &\text{if } |x_i - y_i| < 1\\ |x_i - y_i| - 0.5, \quad &\text{otherwise} \end{cases}\]It also has `reduction` options.
...
\[\text{loss}(x, c) = -\log\left(\frac{\exp(x[c])}{\sum_j \exp(x[j])}\right) = -x[c] + \log\left(\sum_j \exp(x[j])\right)\]or in the case of the `weight` argument being specified:
...
\[H(p,q) = H(p) + \mathcal{D}_{KL} (p \mid\mid q)\]where \(H(p,q) = - \sum_i p(x_i) \log (q(x_i))\) is the cross-entropy (between two distributions), \(H(p) = - \sum_i p(x_i) \log (p(x_i))\) is the entropy, and \(\mathcal{D}_{KL} (p \mid\mid q) = \sum_i p(x_i) \log \frac{p(x_i)}{q(x_i)}\) is the KL divergence.
