---
source_url: "https://atcold.github.io/NYU-DLSP20/en/week09/09-3/"
title: "Deep Learning"
date: "2020-03-31"
extractor: "generic"
---
...
[Introduction to generative adversarial networks (GANs)](https://www.youtube.com/watch?v=xYc11zyZ26M&t=57s)
 ...
 ![](/NYU-DLSP20/images/week09/09-3/GANArchitecture.png) **Fig. 1**: GAN Architecture
...
[Difference between GANs and VAEs](https://www.youtube.com/watch?v=xYc11zyZ26M&t=1844s)
 ...
 ![](/NYU-DLSP20/images/week09/09-3/GANvsVAEArchitecture.png) **Fig. 3**: VAE (left) *vs.* GAN (right) - Architectural design
...
- The generator upsamples the input using several `nn.ConvTranspose2d`modules separated with`nn.BatchNorm2d`and`nn.ReLU`.
- At the end of the sequential, the network uses `nn.Tanh()`to squash outputs to $(-1,1)$.
- The input random vector has size $nz$. The output has size $nc \times 64 \times 64$, where $nc$ is the number of channels.
...
- It is important to use `nn.LeakyReLU`as the activation function to avoid killing the gradients in negative regions. Without these gradients, the generator will not receive updates.
- At the end of the sequential, the discriminator uses `nn.Sigmoid()`to classify the input.
