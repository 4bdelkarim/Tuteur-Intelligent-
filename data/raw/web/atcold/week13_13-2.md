---
source_url: "https://atcold.github.io/NYU-DLSP20/en/week13/13-2/"
title: "Deep Learning"
date: "2020-04-27"
extractor: "generic"
---
...
\[h^{l+1}=\eta\bigg(\boldsymbol{\phi} \Big(\text{diag}(\boldsymbol{B}w^l)\Big)\boldsymbol{\phi^\top} h^l\bigg)\]Now, we only have $O(1)$ parameters (constant $K$) per layer to be learned through backpropagation. However, the learning complexity is still $O(n^2)$.
...
\[w*h=\hat{w}(\Delta)h=\bigg(\sum^{K-1}_{k=0}w_k\Delta^k\bigg)h\]One nice feature is that filters are localized in exactly k-hop supports.

![](/NYU-DLSP20/images/week13/13-2/Figure1.png)**Figure 1:**Demonstrates 1-hop and 2-hop neighbourhoods

We replace the expression $\Delta^kh$ with $X_k$, a recursive equation defined as,

\[X_k=\Delta X_{k-1} \text{ and } X_0=h\]Complexity is now $O(E.K)=O(n)$ for sparse (real-world) graphs. We can reshape $X_k$ into $\bar{X}$ to form a linear operation. We now have the following activation equation.

\[h^{l+1}=\eta\bigg(\sum^{K-1}_{k=0}w_kX_k\bigg)=\eta\Big((w^l)^\top \bar{X}\Big)\]Note: Since no Laplacian eigen-decomposition is used, all operations are in the spatial (not spectral) domain, so calling them Spectral GCNs may be misguided. Further, another drawback of LapGCNs is that convolutional layers involve sparse linear operations, which GPU’s are not fully optimized for.
...
\[X_k=2\tilde{\Delta} X_{k-1} - X_{k-2}, X_0=h, X_1=\tilde{\Delta}h \text{ and } \tilde{\Delta} = 2\lambda_n^{-1}\Delta - \boldsymbol{I}\]Now we have stability under coefficient perturbation.
...
\[\hat{w}(\Delta)=w_0+2\Re\left\{\sum^{K-1}_{k=0}w_k\frac{(z\Delta-i)^k}{(z\Delta+i)^k}\right\}\]CayleyNets have the same properties as ChebNets (are isotropic), but they are localized in frequency (with spectral zoom) and provide a richer class of filters (for the same order $K$).
...
![](/NYU-DLSP20/images/week13/13-2/Figure2.png)**Figure 2:**Template Matching using a template vector

Mathematically, for one feature we have,

\[h_{i}^{l+1}=\eta\bigg(\sum_{j \in N_{i}} \langle w^l,h_{ij}^l \rangle \bigg)\]where, $w^l$ is the template vector at layer $l$ of dimensions $d \times 1$ and $h_{ij}^l$ is the vector at node j with $d \times 1$ which will result in a scalar quantity $h_{i}^{l+1}$ at node $i$.
...
\[h^{l+1} = \eta(\boldsymbol{D}^{-1}\boldsymbol{A}h^{l}\boldsymbol{W}^{l})\]where, $\boldsymbol{A}$ has the dimensions $n \times n$, $h^{l}$ has dimensions $n \times d$ and $W^{l}$ has $d \times d$, which results in a $n \times d$ $h^{l+1}$ matrix.

And the vectorial representation being,

\[h_{i}^{l+1} = \eta\bigg(\frac{1}{d_{i}}\sum_{j \in N_{i}}\boldsymbol{A}_{ij}\boldsymbol{W}^{l}h_{j}^{l}\bigg)\]where, $h_{i}^{l+1}$ has the dimensions of $d \times 1$
...
![](/NYU-DLSP20/images/week13/13-2/Figure4.png)**Figure 3:**Isotropic model \[h_{i}^{l+1} = f_\text{GCN}(h_{i}^{l}, \{h_{j}^l: j \rightarrow i\})\]
...
![](/NYU-DLSP20/images/week13/13-2/Figure3.png)**Figure 4:**GraphSage
...
\[h_{i}^{l+1} = \texttt{ReLU}(\boldsymbol{W}_{2}^{l}\space \texttt{ReLU}(\texttt{BN}(\boldsymbol{W}_{1}^{l} \hat(h_{j}^{l+1})))\]where, $\texttt{BN}$ stands for Batch Normalization.

\[h_{i}^{l+1} = (1 + \epsilon)h_{i}^{l} + \sum_{j \in N_{i}} h_{j}^{l}\] ![](/NYU-DLSP20/images/week13/13-2/Figure5.png)**Figure 5:**Examples of two isomorphic graphs
...
![](/NYU-DLSP20/images/week13/13-2/Figure6.png)**Figure 6:**MoNet

### Graph Attention Networks (GAT)

GAT uses the attention mechanism to introduce anisotropy in the neighbourhood aggregation function.

![](/NYU-DLSP20/images/week13/13-2/Figure7.png)**Figure 7:**GAT
...
![](/NYU-DLSP20/images/week13/13-2/Figure8.png)**Figure 8:**Gated Graph ConvNet

### Graph Transformers

![](/NYU-DLSP20/images/week13/13-2/Figure9.png)**Figure 9:**Graph Transformer
...
**Figure 10:**Graph Regression task - Quantum Chemistry ![](/NYU-DLSP20/images/week13/13-2/Figure11.png)

**Figure 11:**Performance of various GCNs on the regression task
...
**Figure 12:**Graph Classification task ![](/NYU-DLSP20/images/week13/13-2/Figure13.png)

**Figure 13:**Performance of various GCNs on Graph Classification task
...
**Figure 14:**Edge Classification task. ![](/NYU-DLSP20/images/week13/13-2/Figure15.png)

**Figure 15:**Performance of various GCNs on Edge Classification task
...
![](/NYU-DLSP20/images/week13/13-2/Figure16.png)**Figure 16:**Applications
