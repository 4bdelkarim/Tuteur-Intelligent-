---
source_url: "https://atcold.github.io/NYU-DLSP20/en/week15/15-1/"
title: "Deep Learning"
date: "2020-10-18"
extractor: "generic"
---

# Inference for latent variable Energy-Based Models (EBMs)
...
\[\vect{y} = \begin{bmatrix} \rho_1(x)\cos(\theta) + \epsilon \\ \rho_2(x)\sin(\theta) + \epsilon \end{bmatrix},\]where $x \sim \mathcal{U}(0,1),\space \theta \sim \mathcal{U}(0,2\pi),\space \epsilon \sim \mathcal{N}[0, (\frac{1}{20})^2]$ and $\rho : \mathbb{R} \mapsto \mathbb{R}^2$ maps $x$ into \(\begin{bmatrix}\alpha x + \beta (1-x) \\ \beta x + \alpha (1-x) \end{bmatrix}\exp(2x)\).

![](/NYU-DLSP20/images/week15/15-1/Figure1.png)**Figure 1:**3D Visualization of Ellipse Function
...
For simplicity, we fix input $x = 0$ and let $\alpha = 1.5, \beta = 2$, inducing \(\vect{y} = \begin{bmatrix} 2\cos(\theta) + \epsilon \\ 1.5\sin(\theta) + \epsilon \end{bmatrix}\) , from which we randomly sample 24 data points $Y = [\vect{y}^{(1)},\ldots,\vect{y}^{(24)}]$. Meanwhile, we take latent variable $z = [0:\frac{\pi}{24}:2\pi)$ and feed it into a decoder to produce $\tilde{\vect{y}}$ (Figure 2 and 3). Then, the energy function is computed as the squared Euclidean distance between $\vect{y}$ and $\tilde{\vect{y}}$:

\[E(\vect{y},z) \equiv E(\vect{y},\tilde{\vect{y}}(z)) = [y_1 - g_1(z)]^2 + [y_2 - g_2(z)]^2, \space \vect{y} \in Y,\]where $\vect{g} = [g_1 \space\space g_2]^{\top} : \mathbb{R} \mapsto \mathbb{R}^2$ and $\vect{g}(z) = \ [w_1 \cos(z) \space\space w_2 \sin(z)]^{\top}$.

![](/NYU-DLSP20/images/week15/15-1/Figure2.png)**Figure 2:**Sample Visualization ![](/NYU-DLSP20/images/week15/15-1/Figure3.png)

**Figure 3:**Energy Computation Graph
...
![](/NYU-DLSP20/images/week15/15-1/Figure4.png)**Figure 4:**Energy Functions over Samples
...
![](/NYU-DLSP20/images/week15/15-1/Figure5.png)**Figure 5:**Energy Function associated with the 23rd Sample
...
![](/NYU-DLSP20/images/week15/15-1/Figure6.png)**Figure 6:**Relationship between Latent Variable, Decoder Output, and the 23rd Sample
...
\[F_{\infty} \equiv \min_{z} E(\vect{y}, z)\]We will also express the latent variable which yields the free energy as $\check{z}$ defined by the following.

\[\check{z} = \arg \min_{z} E(\vect{y}, z)\]As depicted in Figure 5, to find the free energy associated with $\vect{y}^{(23)}$ we’ll start with an initial latent variable $\tilde{z}$. $\check{z}$ can be evaluated through optimization algorithms such as exhaustive search, conjugate gradient, line search, or limited-memory BFGS. The free energy is the minimum value of the energy with respect to the latent variable.
...
![](/NYU-DLSP20/images/week15/15-1/Figure7.png)**Figure 7:**Free Energy Evaluation Associated with the 23rd Sample
...
![](/NYU-DLSP20/images/week15/15-1/Figure8.png)**Figure 8:**Free Energy Example in Mesh Grid
...
![](/NYU-DLSP20/images/week15/15-1/Figure10.png)**Figure 10:**Energy Functions for the Five Points
...
![](/NYU-DLSP20/images/week15/15-1/Figure11.png)**Figure 11:**Free Energy Heat Map
