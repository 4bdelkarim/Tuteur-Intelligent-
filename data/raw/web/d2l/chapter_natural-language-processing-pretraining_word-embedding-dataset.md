---
source_url: "https://d2l.ai/chapter_natural-language-processing-pretraining/word-embedding-dataset.html"
title: "15.3. The Dataset for Pretraining Word Embeddings"
chapter: "15"
section_number: "15.3"
date: null
extractor: "d2l"
...
Section 15.1 and negative sampling in
Section 15.2 as an example. In this section, we begin
...
(PTB)](https://catalog.ldc.upenn.edu/LDC99T42). This corpus is
...
(Mikolov et al., 2013). Specifically, each indexed
word $w_i$ in the dataset will be discarded with probability

$$
(15.3.1)\[P(w_i) = \max\left(1 - \sqrt{\frac{t}{f(w_i)}}, 0\right),
$$

where $f(w_i)$ is the ratio of the number of words $w_i$ to
the total number of words in the dataset, and the constant $t$ is
a hyperparameter ($10^{-4}$ in the experiment). We can see that
only when the relative frequency $f(w_i) > t$ can the
(high-frequency) word $w_i$ be discarded, and the higher the
...
![../_images/output_word-embedding-dataset_f77071_39_0.svg](../_images/output_word-embedding-dataset_f77071_39_0.svg)
...
For example, we can draw 10 random variables $X$ among indices 1,
2, and 3 with sampling probabilities $P(X=1)=2/9, P(X=2)=3/9$, and
$P(X=3)=4/9$ as follows.
