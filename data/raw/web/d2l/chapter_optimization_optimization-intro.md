---
source_url: "https://d2l.ai/chapter_optimization/optimization-intro.html"
title: "12.1. Optimization and Deep Learning"
chapter: "12"
section_number: "12.1"
date: null
extractor: "d2l"
...
Section 3.6, we discussed the difference
...
Section 4.7.3.1, the empirical risk is an
...
![../_images/output_optimization-intro_70d214_27_0.svg](../_images/output_optimization-intro_70d214_27_0.svg)
...
Section 3.1 we distinguished between analytical
...
For any objective function $f(x)$, if the value of $f(x)$ at
$x$ is smaller than the values of $f(x)$ at any other points
in the vicinity of $x$, then $f(x)$ could be a local
minimum. If the value of $f(x)$ at $x$ is the minimum of the
objective function over the entire domain, then $f(x)$ is the
global minimum.
...
![../_images/output_optimization-intro_70d214_39_0.svg](../_images/output_optimization-intro_70d214_39_0.svg)
...
Consider the function $f(x) = x^3$. Its first and second
derivative vanish for $x=0$. Optimization might stall at this
...
![../_images/output_optimization-intro_70d214_51_0.svg](../_images/output_optimization-intro_70d214_51_0.svg)

Saddle points in higher dimensions are even more insidious, as the
example below shows. Consider the function $f(x, y) = x^2 - y^2$.
It has its saddle point at $(0, 0)$. This is a maximum with
respect to $y$ and a minimum with respect to $x$. Moreover,
...
![../_images/output_optimization-intro_70d214_63_0.svg](../_images/output_optimization-intro_70d214_63_0.svg)

We assume that the input of a function is a $k$-dimensional vector
and its output is a scalar, so its Hessian matrix will have $k$
...
derivatives in Section 5.1.2. For instance,
assume that we want to minimize the function $f(x) = \tanh(x)$ and
we happen to get started at $x = 4$. As we can see, the gradient
of $f$ is close to nil. More specifically,
...
![../_images/output_optimization-intro_70d214_75_0.svg](../_images/output_optimization-intro_70d214_75_0.svg)
...
1. Consider a simple MLP with a single hidden layer of, say, $d$
   ...
   local minimum there are at least $d!$ equivalent solutions that
   behave identically.
2. Assume that we have a symmetric random matrix $\mathbf{M}$
   where the entries $M_{ij} = M_{ji}$ are each drawn from some
   probability distribution $p_{ij}$. Furthermore assume that
   $p_{ij}(x) = p_{ij}(-x)$, i.e., that the distribution is
   symmetric (see e.g., Wigner (1958) for details).

   1. Prove that the distribution over eigenvalues is also symmetric.
      That is, for any eigenvector $\mathbf{v}$ the probability
      that the associated eigenvalue $\lambda$ satisfies
