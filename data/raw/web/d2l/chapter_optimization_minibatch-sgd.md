---
source_url: "https://d2l.ai/chapter_optimization/minibatch-sgd.html"
title: "12.5. Minibatch Stochastic Gradient Descent"
chapter: "12"
section_number: "12.5"
date: null
extractor: "d2l"
...
Section 12.4 processes one training example at a time to make
...
to $2 \cdot 10^9 \cdot 16 \cdot 32 = 10^{12}$ bytes per second.
...
[Wikipedia article](https://en.wikipedia.org/wiki/Cache_hierarchy)
...
$\mathbf{A} = \mathbf{B}\mathbf{C}$. We have a number of options
for calculating $\mathbf{A}$. For instance, we could try the
following:

1. We could compute
   $\mathbf{A}_{ij} = \mathbf{B}_{i,:} \mathbf{C}_{:,j}$, i.e., we
   could compute it elementwise by means of dot products.
2. We could compute
   $\mathbf{A}_{:,j} = \mathbf{B} \mathbf{C}_{:,j}$, i.e., we
   could compute it one column at a time. Likewise we could compute
   $\mathbf{A}$ one row $\mathbf{A}_{i,:}$ at a time.
3. We could simply compute $\mathbf{A} = \mathbf{B} \mathbf{C}$.
4. We could break $\mathbf{B}$ and $\mathbf{C}$ into smaller
   block matrices and compute $\mathbf{A}$ one block at a time.
...
$\mathbf{A}_{ij}$. Even worse, due to the fact that matrix
...
keep the column vector $\mathbf{C}_{:,j}$ in the CPU cache while
we keep on traversing through $\mathbf{B}$. This halves the memory
...
$\mathbf{B}$ and $\mathbf{C}$ respectively to assign the
value to $\mathbf{A}$.
...
$\mathbf{B} \in \mathbb{R}^{m \times n}$ and
$\mathbf{C} \in \mathbb{R}^{n \times p}$ takes approximately
$2mnp$ floating point operations, when scalar multiplication and
addition are counted as separate operations (fused in practice). Thus,
multiplying two $256 \times 256$ matrices takes $0.03$
...
Section 8.5 we used a type of regularization that was
...
Ioffe (2017) for details on how to rescale and compute the
appropriate terms.
...
aircraft](https://archive.ics.uci.edu/ml/datasets/Airfoil+Self-Noise)
to compare these optimization algorithms. For convenience we only use
the first $1,500$ examples. The data is whitened for
...
Section 3.4. In the following we provide a slightly
...
![../_images/output_minibatch-sgd_f4d60f_111_1.svg](../_images/output_minibatch-sgd_f4d60f_111_1.svg)
...
![../_images/output_minibatch-sgd_f4d60f_123_1.svg](../_images/output_minibatch-sgd_f4d60f_123_1.svg)
...
![../_images/output_minibatch-sgd_f4d60f_135_1.svg](../_images/output_minibatch-sgd_f4d60f_135_1.svg)
...
![../_images/output_minibatch-sgd_f4d60f_147_1.svg](../_images/output_minibatch-sgd_f4d60f_147_1.svg)
...
![../_images/output_minibatch-sgd_f4d60f_159_0.svg](../_images/output_minibatch-sgd_f4d60f_159_0.svg)
...
![../_images/output_minibatch-sgd_f4d60f_183_1.svg](../_images/output_minibatch-sgd_f4d60f_183_1.svg)
