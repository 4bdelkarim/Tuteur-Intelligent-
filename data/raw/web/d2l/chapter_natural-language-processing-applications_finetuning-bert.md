...
![../_images/bert-one-seq.svg](../_images/bert-one-seq.svg)
...
![../_images/bert-two-seqs.svg](../_images/bert-two-seqs.svg)
...
![../_images/bert-tagging.svg](../_images/bert-tagging.svg)
...
![../_images/bert-qa.svg](../_images/bert-qa.svg)
...
1. Let’s design a search engine algorithm for news articles. When the
   system receives an query (e.g., “oil industry during the coronavirus
   outbreak”), it should return a ranked list of news articles that are
   most relevant to the query. Suppose that we have a huge pool of news
   articles and a large number of queries. To simplify the problem,
   suppose that the most relevant article has been labeled for each
   query. How can we apply negative sampling (see
   Section 15.2.1) and BERT in the algorithm
   design?
