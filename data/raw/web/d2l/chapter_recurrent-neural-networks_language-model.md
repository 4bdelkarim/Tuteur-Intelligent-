---
source_url: "https://d2l.ai/chapter_recurrent-neural-networks/language-model.html"
title: "9.3. Language Models"
chapter: "9"
section_number: "9.3"
date: null
extractor: "d2l"
...
In Section 9.2, we saw how to map text sequences into
...
token at a time $x_t \sim P(x_t \mid x_{t-1}, \ldots, x_1)$. Quite
...
$$
(9.3.3)\[\begin{split}\begin{aligned}&P(\textrm{deep}, \textrm{learning}, \textrm{is}, \textrm{fun}) \\
...
Gutenberg](https://en.wikipedia.org/wiki/Project_Gutenberg), and all
...
example, the estimate $\hat{P}(\textrm{deep})$ can be calculated
...
where $n(x)$ and $n(x, x')$ are the number of occurrences of
...
empirical results in Section 9.2.5, things take a
...
$$
(9.3.6)\[\begin{split}\begin{aligned}
...
Here $\epsilon_1,\epsilon_2$, and $\epsilon_3$ are
hyperparameters. Take $\epsilon_1$ as an example: when
$\epsilon_1 = 0$, no smoothing is applied; when $\epsilon_1$
approaches positive infinity, $\hat{P}(x)$ approaches the uniform
probability $1/m$. The above is a rather primitive variant of what
...
Section 9.2.5, many $n$-grams occur very
...
(Section 4.1.3). If we want to compress text, we
...
all possible length-$n$ subsequences, we can introduce randomness.
More concretely, at the beginning of each epoch, discard the first
$d$ tokens, where $d\in [0,n)$ is uniformly sampled at
...
input sequence $\mathbf x_t$ is $\mathbf x_{t+1}$ with
length $n$.

![../_images/lang-model-data.svg](../_images/lang-model-data.svg)

Fig. 9.3.1 Obtaining five pairs of input sequences and target sequences from
partitioned length-5 subsequences.

Fig. 9.3.1 shows an example of obtaining five pairs
of input sequences and target sequences with $n=5$ and
$d=2$.
...
long sequences, $n$-grams provide a convenient model by truncating
...
Section 11.9), at the present moment
