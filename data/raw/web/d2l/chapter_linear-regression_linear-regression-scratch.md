---
source_url: "https://d2l.ai/chapter_linear-regression/linear-regression-scratch.html"
title: "3.4. Linear Regression Implementation from Scratch"
chapter: "3"
section_number: "3.4"
date: null
extractor: "d2l"
...
Section 3.3 and apply our model on the
...
output. Using the same notation as (3.1.4) for our
linear model we simply take the matrix–vector product of the input
features $\mathbf{X}$ and the model weights $\mathbf{w}$,
and add the offset $b$ to each example. The product
$\mathbf{Xw}$ is a vector and $b$ is a scalar. Because of
the broadcasting mechanism (see Section 2.1.4), when
...
squared loss function in (3.1.5). In the implementation, we
...
As discussed in Section 3.1, linear regression has
...
(introduced in Section 3.2.1), to have a similar API
as the built-in SGD optimizer. We update the parameters in the `step`
method. The `zero_grad` method sets all gradients to 0, which must be
run before a backpropagation step.
...
Section 3.3 does not provide a validation
...
![../_images/output_linear-regression-scratch_b94b4e_111_0.svg](../_images/output_linear-regression-scratch_b94b4e_111_0.svg)
...
(Vapnik, 1992). Fortunately, even on difficult optimization
...
1. What would happen if we were to initialize the weights to zero. Would
   ...
   variance $1000$ rather than $0.01$?
2. Assume that you are [Georg Simon
   Ohm](https://en.wikipedia.org/wiki/Georg_Ohm) trying to come up
...
3. Can you use [Planck’s
   Law](https://en.wikipedia.org/wiki/Planck%27s_law) to determine
   the temperature of an object using spectral energy density? For
   reference, the spectral density $B$ of radiation emanating from
   a black body is
   $B(\lambda, T) = \frac{2 hc^2}{\lambda^5} \cdot \left(\exp \frac{h c}{\lambda k T} - 1\right)^{-1}$.
   Here $\lambda$ is the wavelength, $T$ is the temperature,
   $c$ is the speed of light, $h$ is Planck’s constant, and
   $k$ is the Boltzmann constant. You measure the energy for
   different wavelengths $\lambda$ and you now need to fit the
...
8. Try implementing a different loss function, such as the absolute
   ...
   2. Check whether there is a difference in behavior if you actively
      perturb some entries, such as $y_5 = 10000$, of
      $\mathbf{y}$.
