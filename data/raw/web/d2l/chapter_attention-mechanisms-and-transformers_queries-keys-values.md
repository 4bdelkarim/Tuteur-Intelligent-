---
source_url: "https://d2l.ai/chapter_attention-mechanisms-and-transformers/queries-keys-values.html"
title: "11.1. Queries, Keys, and Values"
chapter: "11"
section_number: "11.1"
date: null
extractor: "d2l"
...
kernels (Kalchbrenner et al., 2014). This approach
can lead to significant problems when the input is truly of varying size
with varying information content, such as in Section 10.7 in
the transformation of text (Sutskever et al., 2014). In
...
Yang *et al.* (2016) only offer limited benefit.

Compare this to databases. In their simplest form they are collections
of keys ($k$) and values ($v$). For instance, our database
$\mathcal{D}$ might consist of tuples {(“Zhang”, “Aston”),
(“Lipton”, “Zachary”), (“Li”, “Mu”), (“Smola”, “Alex”), (“Hu”,
“Rachel”), (“Werness”, “Brent”)} with the last name being the key and
the first name being the value. We can operate on $\mathcal{D}$,
for instance with the exact query ($q$) for “Li” which would
return the value “Mu”. If (“Li”, “Mu”) was not a record in
$\mathcal{D}$, there would be no valid answer. If we also allowed
...
- We can design queries $q$ that operate on
  ($k$,$v$) pairs in such a manner as to be valid
...
decade: the *attention mechanism* (Bahdanau et al., 2014). We
...
($i = 1, \ldots, m$) are scalar attention weights. The operation
...
attention to the terms for which the weight $\alpha$ is
significant (i.e., large). As such, the attention over
$\mathcal{D}$ generates a linear combination of values contained
...
- The weights $\alpha(\mathbf{q}, \mathbf{k}_i)$ are nonnegative.
  ...
  the convex cone spanned by the values $\mathbf{v}_i$.
- The weights $\alpha(\mathbf{q}, \mathbf{k}_i)$ form a convex
  combination, i.e.,
  $\sum_i \alpha(\mathbf{q}, \mathbf{k}_i) = 1$ and
  $\alpha(\mathbf{q}, \mathbf{k}_i) \geq 0$ for all $i$.
  This is the most common setting in deep learning.
- Exactly one of the weights $\alpha(\mathbf{q}, \mathbf{k}_i)$
  is $1$, while all others are $0$. This is akin to a
  traditional database query.
- All weights are equal, i.e.,
  $\alpha(\mathbf{q}, \mathbf{k}_i) = \frac{1}{m}$ for all
  $i$. This amounts to averaging across the entire database, also
  called average pooling in deep learning.

A common strategy for ensuring that the weights sum up to $1$ is
...
$a(\mathbf{q}, \mathbf{k})$ and then apply the softmax operation
...
reinforcement learning methods (Mnih et al., 2014). As
...
Fig. 11.1.1. We thus focus our exposition on this family of
differentiable mechanisms.

![../_images/qkv.svg](../_images/qkv.svg)

Fig. 11.1.1 The attention mechanism computes a linear combination over values
$\mathbf{v}_\mathit{i}$ via attention pooling, where weights
are derived according to the compatibility between a query
$\mathbf{q}$ and keys $\mathbf{k}_\mathit{i}$.
...
$1$. In this case we might *interpret* large weights as a way for
...
representing a case where the attention weight is $1$ only when
...
![../_images/output_queries-keys-values_7fe0e8_33_0.svg](../_images/output_queries-keys-values_7fe0e8_33_0.svg)
...
Nadaraya–Watson estimator (Nadaraya, 1964, Watson, 1964) that we
...
2. Suppose that the attention function is given by
   ...
   and that $\mathbf{k}_i = \mathbf{v}_i$ for
   $i = 1, \ldots, m$. Denote by
   $p(\mathbf{k}_i; \mathbf{q})$ the probability distribution over
...
4. Review the design of the Squeeze and Excitation Networks
   (Hu et al., 2018) and interpret them through the lens of the
   attention mechanism.
