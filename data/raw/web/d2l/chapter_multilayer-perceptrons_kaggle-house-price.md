---
source_url: "https://d2l.ai/chapter_multilayer-perceptrons/kaggle-house-price.html"
title: "5.7. Predicting House Prices on Kaggle"
chapter: "5"
section_number: "5.7"
date: null
extractor: "d2l"
...
De Cock (2011), covers house prices in Ames, Iowa from the
...
[Kaggle](https://www.kaggle.com) is a popular platform that hosts
...
Fig. 5.7.2, you can find the dataset (under the
...
on the competition tab in Fig. 5.7.2 has links for
downloading the data.

To get started, we will read in and process the data using `pandas`,
which we introduced in Section 2.2. For convenience, we can
...
where $\mu$ and $\sigma$ denote mean and standard deviation,
...
$E[\frac{x-\mu}{\sigma}] = \frac{\mu - \mu}{\sigma} = 0$ and that
...
Section 4.1.1). For instance, “MSZoning”
assumes the values “RL” and “RM”. Dropping the “MSZoning” feature, two
new indicator features “MSZoning\_RL” and “MSZoning\_RM” are created with
values being either 0 or 1. According to one-hot encoding, if the
original value of “MSZoning” is “RL”, then “MSZoning\_RL” is 1 and
“MSZoning\_RM” is 0. The `pandas` package does this automatically for
us.
...
absolute error $y - \hat{y}$. For instance, if our prediction is
...
submissions. After all, a small value $\delta$ for
$|\log y - \log \hat{y}| \leq \delta$ translates into
$e^{-\delta} \leq \frac{\hat{y}}{y} \leq e^\delta$. This leads to
...
Section 3.6.3, where we discussed
...
function that returns the $i^\textrm{th}$ fold of the data in a
$K$-fold cross-validation procedure. It proceeds by slicing out
the $i^\textrm{th}$ segment as validation data and returning the
...
The average validation error is returned when we train $K$ times
...
enough dataset, and the normal sorts of hyperparameters, $K$-fold
...
![../_images/output_kaggle-house-price_1852a7_88_1.svg](../_images/output_kaggle-house-price_1852a7_88_1.svg)
...
$K$-fold cross-validation grows considerably higher. This
...
$K$ models. Saving the predictions in a csv file will simplify
...
Next, as demonstrated in Fig. 5.7.3, we can submit
...
and adjust the hyperparameters, we can use $K$-fold
cross-validation .
