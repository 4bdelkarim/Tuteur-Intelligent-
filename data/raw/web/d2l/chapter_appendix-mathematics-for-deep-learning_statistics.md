---
source_url: "https://d2l.ai/chapter_appendix-mathematics-for-deep-learning/statistics.html"
title: "22.10. Statistics"
chapter: "22"
section_number: "22.10"
date: null
extractor: "d2l"
---
...
scholar Al-Kindi in the $9^{\textrm{th}}$-century, who gave a
...
Section 22.7. If you have a number of samples from
...
![../_images/output_statistics_bc7dd5_3_0.svg](../_images/output_statistics_bc7dd5_3_0.svg)
...
squared error (MSE)* (or $l_2$ loss) estimator which can be
defined as
...
$\hat{\theta}_n$, the mathematical illustration of *statistical
...
Note that when $\textrm{bias}(\hat{\theta}_n) = 0$, the
...
$$
(22.10.4)\[\begin{split}\begin{aligned}
...
result from noise in the $\theta$ itself.
...
Next, we calculate
$\textrm{Var} (\hat{\theta}_n) + [\textrm{bias} (\hat{\theta}_n)]^2$
...
the *null hypothesis* $H_0$, which we try to reject using the
...
rejecting the null hypothesis, $H_0$, when it should not be
rejected, i.e.,

$$
(22.10.5)\[\textrm{statistical significance }= 1 - \alpha = 1 - P(\textrm{reject } H_0 \mid H_0 \textrm{ is true} ).
$$

It is also referred to as the *type I error* or *false positive*. The
$\alpha$, is called as the *significance level* and its commonly
used value is $5\%$, i.e., $1-\alpha = 95\%$. The
...
![../_images/statistical-significance.svg](../_images/statistical-significance.svg)
...
$$
(22.10.6)\[\textrm{statistical power }= 1 - \beta = 1 - P(\textrm{ fail to reject } H_0  \mid H_0 \textrm{ is false} ).
$$
...
A *test statistic* $T(x)$ is a scalar which summarizes some
...
The $p$-value (or the *probability value*) is the probability that
$T(X)$ is at least as extreme as the observed test statistic
$T(x)$ assuming that the null hypothesis is *true*, i.e.,

$$
(22.10.7)\[p\textrm{-value} = P_{H_0}(T(X) \geq T(x)).
$$

If the $p$-value is smaller than or equal to a predefined and
...
2. Set the statistical significance level $\alpha$ and a
...
Probability” by Jerzy Neyman (Neyman, 1937), who first introduced
...
Note that (22.10.8) is about variable $C_n$, not
...
(22.10.8) by constructing intervals that we are *almost
...
$$
(22.10.9)\[\hat\mu_n = \frac{1}{n}\sum_{i=1}^n x_i \;\textrm{and}\; \hat\sigma^2_n = \frac{1}{n-1}\sum_{i=1}^n (x_i - \hat\mu)^2.
$$
...
the *Student’s t-distribution on* $n-1$ *degrees of freedom*.
...
`t_star` obtained either programmatically or from a $t$-table.
...
1. Let
   $X_1, X_2, \ldots, X_n \overset{\textrm{iid}}{\sim} \textrm{Unif}(0, \theta)$,
...
2. For our chemist example in introduction, can you derive the 5 steps
   ...
   $1 - \beta = 0.8$.
3. Run the confidence interval code with $N=2$ and
