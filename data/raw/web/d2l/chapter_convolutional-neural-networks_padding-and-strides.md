---
source_url: "https://d2l.ai/chapter_convolutional-neural-networks/padding-and-strides.html"
title: "7.3. Padding and Stride"
chapter: "7"
section_number: "7.3"
date: null
extractor: "d2l"
...
Recall the example of a convolution in Fig. 7.2.1. The
...
dimension $2\times2$. Assuming that the input shape is
$n_\textrm{h}\times n_\textrm{w}$ and the convolution kernel shape
is $k_\textrm{h}\times k_\textrm{w}$, the output shape will be
...
width and height greater than $1$, after applying many successive
convolutions, we tend to wind up with outputs that are considerably
smaller than our input. If we start with a $240 \times 240$ pixel
image, ten layers of $5 \times 5$ convolutions reduce the image to
$200 \times 200$ pixels, slicing off $30 \%$ of the image
...
Fig. 7.3.1 that depicts the pixel utilization as a
...
![../_images/conv-reuse.svg](../_images/conv-reuse.svg)

Fig. 7.3.1 Pixel utilization for convolutions of size $1 \times 1$,
$2 \times 2$, and $3 \times 3$ respectively.
...
of the extra pixels to zero. In Fig. 7.3.2, we pad a
$3 \times 3$ input, increasing its size to $5 \times 5$. The
corresponding output then increases to a $4 \times 4$ matrix. The
...
![../_images/conv-pad.svg](../_images/conv-pad.svg)
...
$p_\textrm{w}$ columns of padding (roughly half on the left and
...
$p_\textrm{h}$ and $p_\textrm{w}$, respectively.

In many cases, we will want to set $p_\textrm{h}=k_\textrm{h}-1$
and $p_\textrm{w}=k_\textrm{w}-1$ to give the input and output the
...
$k_\textrm{h}$ is odd here, we will pad $p_\textrm{h}/2$
rows on both sides of the height. If $k_\textrm{h}$ is even, one
possibility is to pad $\lceil p_\textrm{h}/2\rceil$ rows on the
top of the input and $\lfloor p_\textrm{h}/2\rfloor$ rows on the
...
Fig. 7.3.3 shows a two-dimensional cross-correlation
...
$0\times0+6\times1+0\times2+0\times3=6$. We can see that when the
...
![../_images/conv-stride.svg](../_images/conv-stride.svg)

Fig. 7.3.3 Cross-correlation with strides of 3 and 2 for height and width,
respectively.

In general, when the stride for the height is $s_\textrm{h}$ and
the stride for the width is $s_\textrm{w}$, the output shape is
...
If we set $p_\textrm{h}=k_\textrm{h}-1$ and
$p_\textrm{w}=k_\textrm{w}-1$, then the output shape can be
simplified to
...
$p_\textrm{h} = p_\textrm{w}$, in which case we simply state that
we choose padding $p$.

A similar convention applies to strides. When horizontal stride
$s_\textrm{h}$ and vertical stride $s_\textrm{w}$ match, we
simply talk about stride $s$. The stride can reduce the resolution
of the output, for example reducing the height and width of the output
to only $1/n$ of the height and width of the input for
...
Alsallakh *et al.* (2020) provided an extensive
...
1. Given the final code example in this section with kernel size
   $(3, 5)$, padding $(0, 1)$, and stride $(3, 4)$,
...
6. How would you implement a stride of $\frac{1}{2}$? What does it
