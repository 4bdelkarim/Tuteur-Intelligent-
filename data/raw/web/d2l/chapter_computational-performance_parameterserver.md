---
source_url: "https://d2l.ai/chapter_computational-performance/parameterserver.html"
title: "13.7. Parameter Servers"
chapter: "13"
section_number: "13.7"
date: null
extractor: "d2l"
...
Let’s review the data parallel training approach to distributed
...
of memory nowadays. Fig. 13.7.1 describes the
variant of data parallelism that we implemented in
Section 13.5. The key aspect in it is that the aggregation
of gradients occurs on one single GPU (GPU 0) before the updated
parameters are rebroadcast to all GPUs.

![../_images/ps.svg](../_images/ps.svg)

Fig. 13.7.1 Left: single GPU training. Right: a variant of multi-GPU training:
...
$\mathbf{g}_1, \ldots, \mathbf{g}_4$ we could aggregate the
gradients on one GPU for each $\mathbf{g}_i$
($i = 1, \ldots, 4$).
...
[multiplexer](https://www.broadcom.com/products/pcie-switches-bridges/pcie-switches).
...
![../_images/bw-hierarchy.svg](../_images/bw-hierarchy.svg)
...
![../_images/ps-distributed.svg](../_images/ps-distributed.svg)
...
computing them for others. See e.g., Sergeev and Del Balso (2018)
...
connectivity structure of Fig. 13.7.4. Each GPU connects to a
...
![../_images/nvlink.svg](../_images/nvlink.svg)
...
![../_images/nvlink-twoloop.svg](../_images/nvlink-twoloop.svg)
...
![../_images/ringsync.svg](../_images/ringsync.svg)

Fig. 13.7.6 Ring synchronization across 4 nodes. Each node starts transmitting
...
$2 \cdot 160 \textrm{MB} / (3 \cdot 18 \textrm{GB/s}) \approx 6 \textrm{ms}$.
...
1. A (different) batch of data is read on each machine, split across
...
![../_images/ps-multimachine.svg](../_images/ps-multimachine.svg)
...
![../_images/ps-multips.svg](../_images/ps-multips.svg)

Fig. 13.7.8 Top: a single parameter server is a bottleneck since its bandwidth is
...
Across many workers and many GPUs the computation for gradient $i$
can be defined as
