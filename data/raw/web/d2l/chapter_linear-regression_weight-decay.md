---
source_url: "https://d2l.ai/chapter_linear-regression/weight-decay.html"
...
(Section 3.6.2.1) we could limit our model’s
...
$d$ grows larger. Given $k$ variables, the number of
monomials of degree $d$ is ${k - 1 + d} \choose {k - 1}$.
...
More commonly called $\ell_2$ regularization outside of deep
...
of its weight vector, e.g., $\| \mathbf{w} \|^2$. Recall that we
introduced the $\ell_2$ norm and $\ell_1$ norm, which are
special cases of the more general $\ell_p$ norm, in
Section 2.3.11. The most common method for
...
$\| \mathbf{w} \|^2$ rather than minimizing the training error.
That is exactly what we want. To illustrate things in code, we revive
our previous example from Section 3.1 for linear
...
is the label for any data example $i$, and $(\mathbf{w}, b)$
...
trade-off via the *regularization constant* $\lambda$, a
...
For $\lambda = 0$, we recover our original loss function. For
$\lambda > 0$, we restrict the size of $\| \mathbf{w} \|$.
We divide by $2$ by convention: when we take the derivative of a
quadratic function, the $2$ and $1/2$ cancel out, ensuring
...
computational convenience. By squaring the $\ell_2$ norm, we
...
Moreover, you might ask why we work with the $\ell_2$ norm in the
first place and not, say, the $\ell_1$ norm. In fact, other
choices are valid and popular throughout statistics. While
$\ell_2$-regularized linear models constitute the classic *ridge
regression* algorithm, $\ell_1$-regularized linear regression is a
similarly fundamental method in statistics, popularly known as *lasso
regression*. One reason to work with the $\ell_2$ norm is that it
...
$\ell_1$ penalties lead to models that concentrate weights on a
...
Using the same notation in (3.1.11), minibatch
stochastic gradient descent updates for $\ell_2$-regularized
...
although $\ell_2$ regularization may not be equivalent to weight
...
$$
(3.7.4)\[y = 0.05 + \sum_{i = 1}^d 0.01 x_i + \epsilon \textrm{ where }
...
our problem to $d = 200$ and working with a small training set
...
squared $\ell_2$ penalty to the original loss function.
...
changed since Section 3.4, so we will just define a
...
![../_images/output_weight-decay_63fc98_78_1.svg](../_images/output_weight-decay_63fc98_78_1.svg)
...
![../_images/output_weight-decay_63fc98_93_1.svg](../_images/output_weight-decay_63fc98_93_1.svg)
...
![../_images/output_weight-decay_63fc98_126_1.svg](../_images/output_weight-decay_63fc98_126_1.svg)
...
(RKHS)](https://en.wikipedia.org/wiki/Reproducing_kernel_Hilbert_space)
...
$\ell_2$ penalty. This leads to weight decay in the update steps
...
1. Experiment with the value of $\lambda$ in the estimation
   problem in this section. Plot training and validation accuracy as a
   function of $\lambda$. What do you observe?
2. Use a validation set to find the optimal value of $\lambda$. Is
   it really the optimal value? Does this matter?
3. What would the update equations look like if instead of
   $\|\mathbf{w}\|^2$ we used $\sum_i |w_i|$ as our penalty
   of choice ($\ell_1$ regularization)?
...
6. In Bayesian statistics we use the product of prior and likelihood to
   ...
   $P(w \mid x) \propto P(x \mid w) P(w)$. How can you identify
   $P(w)$ with regularization?
