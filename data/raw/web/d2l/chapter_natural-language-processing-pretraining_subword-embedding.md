---
source_url: "https://d2l.ai/chapter_natural-language-processing-pretraining/subword-embedding.html"
title: "15.6. Subword Embedding"
chapter: "15"
section_number: "15.6"
date: null
extractor: "d2l"
...
character $n$-gram (Bojanowski et al., 2017).
...
other subwords. Then, extract character $n$-grams from the word.
For example, when $n=3$, we obtain all subwords of length 3:
“<wh”, “whe”, “her”, “ere”, “re>”, and the special subword “<where>”.

In fastText, for any word $w$, denote by $\mathcal{G}_w$ the
...
Letting $\mathbf{z}_g$ be the vector of subword $g$ in the
dictionary, the vector $\mathbf{v}_w$ for word $w$ as a
...
lengths, such as $3$ to $6$, thus the vocabulary size cannot
...
(Radford et al., 2019) and RoBERTa
(Liu et al., 2019). In the following, we will illustrate
...
1. As an example, there are about $3\times 10^8$ possible
   $6$-grams in English. What is the issue when there are too many
...
2. How to design a subword embedding model based on the continuous
   bag-of-words model?
3. To get a vocabulary of size $m$, how many merging operations
   are needed when the initial symbol vocabulary size is $n$?
