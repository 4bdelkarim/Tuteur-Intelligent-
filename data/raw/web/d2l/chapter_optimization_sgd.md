---
source_url: "https://d2l.ai/chapter_optimization/sgd.html"
title: "12.4. Stochastic Gradient Descent"
chapter: "12"
section_number: "12.4"
date: null
extractor: "d2l"
...
![../_images/output_sgd_baca77_15_1.svg](../_images/output_sgd_baca77_15_1.svg)
...
$$
(12.4.5)\[\begin{split}\begin{aligned}
...
![../_images/output_sgd_baca77_27_1.svg](../_images/output_sgd_baca77_27_1.svg)
...
solution $\mathbf{x} = (0, 0)$. Even after 1000 iteration steps
...
![../_images/output_sgd_baca77_39_1.svg](../_images/output_sgd_baca77_39_1.svg)
...
notes](https://www.stat.cmu.edu/%7Eryantibs/convexopt-F15/lectures/26-nonconvex.pdf)
of Tibshirani 2015.
...
simplest proofs (Nesterov and Vial, 2000). Significantly more
...
$f(\boldsymbol{\xi}, \mathbf{x})$ is convex in $\mathbf{x}$
for all $\boldsymbol{\xi}$. More concretely, we consider the
...
where $f(\boldsymbol{\xi}_t, \mathbf{x})$ is the objective
function with respect to the training example $\boldsymbol{\xi}_t$
drawn from some distribution at step $t$ and $\mathbf{x}$ is
...
the expected risk and by $R^*$ its minimum with regard to
$\mathbf{x}$. Last let $\mathbf{x}^*$ be the minimizer (we
assume that it exists within the domain where $\mathbf{x}$ is
defined). In this case we can track the distance between the current
parameter $\mathbf{x}_t$ at time $t$ and the risk minimizer
$\mathbf{x}^*$ and see whether it improves over time:
...
We assume that the $\ell_2$ norm of stochastic gradient
$\partial_\mathbf{x} f(\boldsymbol{\xi}_t, \mathbf{x})$ is bounded
by some constant $L$, hence we have that
...
a bound on the distance between parameters at time $t+1$ as
follows:
...
$t \in \{1, \ldots, T\}$. Since the sum telescopes and by dropping
...
Note that we exploited that $\mathbf{x}_1$ is given and thus the
...
by Jensen’s inequality (setting $i=t$,
$\alpha_i = \eta_t/\sum_{t=1}^T \eta_t$ in
(12.2.3)) and convexity of $R$ it follows
that $E[R(\mathbf{x}_t)] \geq E[R(\bar{\mathbf{x}})]$, thus

$$
(12.4.16)\[\sum_{t=1}^T \eta_t E[R(\mathbf{x}_t)] \geq \sum_{t=1}^T \eta_t  E\left[R(\bar{\mathbf{x}})\right].
$$

Plugging this into the inequality (12.4.13) yields the
bound
...
where
$r^2 \stackrel{\textrm{def}}{=} \|\mathbf{x}_1 - \mathbf{x}^*\|^2$
...
norm of stochastic gradient is bounded ($L$) and how far away from
optimality the initial parameter value is ($r$). Note that the
bound is in terms of $\bar{\mathbf{x}}$ rather than
$\mathbf{x}_T$. This is the case since $\bar{\mathbf{x}}$ is
a smoothed version of the optimization path. Whenever $r, L$, and
$T$ are known we can pick the learning rate
$\eta = r/(L \sqrt{T})$. This yields as upper bound
$rL/\sqrt{T}$. That is, we converge with rate
$\mathcal{O}(1/\sqrt{T})$ to the optimal solution.
...
$x_i$, typically with labels $y_i$ from some distribution
$p(x, y)$ and that we use this to update the model parameters in
...
for some functions $\delta_{x_i}$ and $\delta_{y_i}$ allows
...
$$
(12.4.18)\[P(\textrm{choose~} i) = 1 - P(\textrm{omit~} i) = 1 - (1-1/n)^n \approx 1-e^{-1} \approx 0.63.
$$
...
2. Prove that for the function $f(x_1, x_2) = x_1^2 + 2 x_2^2$
   adding normal noise to the gradient is equivalent to minimizing a
   loss function
   $f(\mathbf{x}, \mathbf{w}) = (x_1 - w_1)^2 + 2 (x_2 - w_2)^2$
   where $\mathbf{x}$ is drawn from a normal distribution.
3. Compare convergence of stochastic gradient descent when you sample
   from $\{(x_1, y_1), \ldots, (x_n, y_n)\}$ with replacement and
...
5. Assume that $f(x) = x^2 (1 + \sin x)$. How many local minima
   does $f$ have? Can you change $f$ in such a way that to
