---
source_url: "https://d2l.ai/chapter_generative-adversarial-networks/gan.html"
title: "20.1. Generative Adversarial Networks"
chapter: "20"
section_number: "20.1"
date: null
extractor: "d2l"
...
(GANs) (Goodfellow et al., 2014), a clever new
...
question whether datasets $X=\{x_1,\ldots, x_n\}$ and
$X'=\{x'_1,\ldots, x'_n\}$ were drawn from the same distribution.
...
test](https://en.wikipedia.org/wiki/Two-sample_hypothesis_testing) to
...
![../_images/gan.svg](../_images/gan.svg)

Fig. 20.1.1 Generative Adversarial Networks

The GAN architecture is illustrated in Fig. 20.1.1. As you can
...
$x$ is real (from real data) or fake (from the generator).
Typically, the discriminator outputs a scalar prediction
$o\in\mathbb R$ for input $\mathbf x$, such as using a fully
connected layer with hidden size 1, and then applies sigmoid function to
obtain the predicted probability $D(\mathbf x) = 1/(1+e^{-o})$.
Assume the label $y$ for the true data is $1$ and $0$
...
$\mathbf z\in\mathbb R^d$ from a source of randomness, *e.g.*, a
normal distribution $\mathbf z \sim \mathcal{N} (0, 1)$. We often
call $\mathbf z$ as the latent variable. It then applies a
function to generate $\mathbf x'=G(\mathbf z)$. The goal of the
generator is to fool the discriminator to classify
$\mathbf x'=G(\mathbf z)$ as true data, *i.e.*, we want
$D( G(\mathbf z)) \approx 1$. In other words, for a given
discriminator $D$, we update the parameters of the generator
$G$ to maximize the cross-entropy loss when $y=0$, *i.e.*,
...
$D(\mathbf x')\approx 1$, so the above loss is near 0, which
...
which is just feeding $\mathbf x'=G(\mathbf z)$ into the
discriminator but giving label $y=1$.

To sum up, $D$ and $G$ are playing a “minimax” game with the
...
arbitrary way with mean $b$ and covariance matrix $A^TA$.
...
![../_images/output_gan_bc32f8_27_1.svg](../_images/output_gan_bc32f8_27_1.svg)
...
but change the label of the fake data from $0$ to $1$.
...
![../_images/output_gan_bc32f8_102_1.svg](../_images/output_gan_bc32f8_102_1.svg)
