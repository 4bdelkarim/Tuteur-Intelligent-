---
source_url: "https://d2l.ai/chapter_appendix-mathematics-for-deep-learning/multivariable-calculus.html"
title: "22.4. Multivariable Calculus"
chapter: "22"
section_number: "22.4"
date: null
extractor: "d2l"
...
What Section 22.3 tells us is that if we
...
$\frac{\partial}{\partial w_1}$ for the derivative in
(22.4.1).

Now, let’s take this and change $w_2$ a little bit to
$w_2 + \epsilon_2$:

$$
(22.4.2)\[\begin{split}\begin{aligned}
...
We have again used the idea that $\epsilon_1\epsilon_2$ is a
higher order term that we can discard in the same way we could discard
$\epsilon^{2}$ in the previous section, along with what we saw in
(22.4.1). By continuing in this manner, we may write that
...
Equation (22.4.5) is worth pondering for a moment. It has
...
approximately how the function $L$ will change given any
...
$$
(22.4.6)\[f(x, y) = \log(e^x + e^y) \textrm{ with gradient } \nabla f (x, y) = \left[\frac{e^x}{e^x+e^y}, \frac{e^y}{e^x+e^y}\right].
$$

If we look at a point like $(0, \log(2))$, we see that

$$
(22.4.7)\[f(x, y) = \log(3) \textrm{ with gradient } \nabla f (x, y) = \left[\frac{1}{3}, \frac{2}{3}\right].
$$

Thus, if we want to approximate $f$ at
$(\epsilon_1, \log(2) + \epsilon_2)$, we see that we should have
the specific instance of (22.4.5):
...
Consider the expression from (22.4.5) again:
...
$L$. Let’s understand geometrically the algorithm of gradient
descent first described in Section 2.5. What we will do is
the following:

1. Start with a random choice for the initial parameters
   $\mathbf{w}$.
2. Find the direction $\mathbf{v}$ that makes $L$ decrease
   the most rapidly at $\mathbf{w}$.
...
$\mathbf{v}$ in the second step. We will call such a direction the
*direction of steepest descent*. Using the geometric understanding of
dot products from Section 22.1, we see
that we can rewrite (22.4.5) as
...
convenience, and used $\theta$ for the angle between
$\mathbf{v}$ and $\nabla_{\mathbf{w}} L(\mathbf{w})$. If we
want to find the direction that decreases $L$ as rapidly as
...
$\cos(\theta)$, and thus we wish to make this cosine as negative
as possible. Now, recalling the shape of cosine, we can make this as
negative as possible by making $\cos(\theta) = -1$ or equivalently
making the angle between the gradient and our chosen direction to be
$\pi$ radians, or equivalently $180$ degrees. The only way
...
Suppose that we wish to find the value of $\mathbf{x}_0$ which
minimizes some function $L(\mathbf{x})$. Let’s suppose that
moreover someone gives us a value and tells us that it is the value that
minimizes $L$. Is there anything we can check to see if their
answer is even plausible?

Again consider (22.4.5):
...
a value of $L$ that is smaller. Thus, if we truly are at a
minimum, this cannot be the case! We can conclude that if
$\mathbf{x}_0$ is a minimum, then
...
The only possible location of minima are at $x = -1, 0, 2$, where
the function takes the values $-5,0, -32$ respectively, and thus
we can conclude that we minimize our function when $x = 2$. A
...
![../_images/output_multivariable-calculus_bf678a_15_0.svg](../_images/output_multivariable-calculus_bf678a_15_0.svg)
...
($w, x, y$, and $z$) which we can make by composing many
terms:
...
Fig. 22.4.1 if we take a look at what variables directly
relate to one another.

![../_images/chain-net1.svg](../_images/chain-net1.svg)

Fig. 22.4.1 The function relations above where nodes represent values and edges
show functional dependence.

Nothing stops us from just composing everything from
(22.4.14) and writing out that
...
$$
(22.4.16)\[\begin{split}\begin{aligned}
...
If we then also wanted to compute $\frac{\partial f}{\partial x}$,
...
$f$ changes when we change $a$, essentially assuming that
$w, x, y$, and $z$ all do not exist. We will reason as we
did back when we worked with the gradient for the first time. Let’s take
$a$ and add a small amount $\epsilon$ to it.

$$
(22.4.17)\[\begin{split}\begin{aligned}
...
the expression $\frac{\partial f}{\partial u}(u(a, b), v(a, b))$,
...
understand how a function of the form $f(u(a, b), v(a, b))$
changes its value with a change in $a$. There are two pathways
this can occur: there is the pathway where
$a \rightarrow u \rightarrow f$ and where
$a \rightarrow v \rightarrow f$. We can compute both of these
contributions via the chain rule:
$\frac{\partial w}{\partial u} \cdot \frac{\partial u}{\partial x}$
and
$\frac{\partial w}{\partial v} \cdot \frac{\partial v}{\partial x}$
...
![../_images/chain-net2.svg](../_images/chain-net2.svg)

Fig. 22.4.2 Another more subtle example of the chain rule.

To compute something like $\frac{\partial f}{\partial y}$, we need
to sum over all (in this case $3$) paths from $y$ to
$f$ giving
...
various architectural choices like those in LSTMs (Section 10.1)
or residual layers (Section 8.6) can help shape the learning
process by controlling gradient flow.

## 22.4.5. The Backpropagation Algorithm

Let’s return to the example of (22.4.14) the previous
section where

$$
(22.4.20)\[\begin{split}\begin{aligned}
f(u, v) & = (u+v)^{2} \\
...
If we want to compute say $\frac{\partial f}{\partial w}$ we may
apply the multi-variate chain rule to see:

$$
(22.4.21)\[\begin{split}\begin{aligned}
...
$\frac{\partial f}{\partial w}$. Notice that all we need here are
the various single step partials:

$$
(22.4.22)\[\begin{split}\begin{aligned}
...
like $\frac{\partial f}{\partial x}$. The reason for that is the
*way* we chose to apply the chain rule. If we look at what we did above,
we always kept $\partial w$ in the denominator when we could. In
this way, we chose to apply the chain rule seeing how $w$ changed
...
apply the chain rule keeping $\partial f$ in the numerator
whenever we can!

To be more explicit, note that we can write

$$
(22.4.23)\[\begin{split}\begin{aligned}
...
$\frac{\partial f}{\partial u}, \frac{\partial f}{\partial v}, \frac{\partial f}{\partial a}, \frac{\partial f}{\partial b}, \; \textrm{and} \; \frac{\partial f}{\partial w}$.
Nothing stops us from also including the equations:

$$
(22.4.24)\[\begin{split}\begin{aligned}
...
and then keeping track of how $f$ changes when we change *any*
...
The fact that we compute derivatives from $f$ back towards the
...
Compute the gradient of $f$ from back to front. We call this the
*backwards pass*.
...
$f(x_1, \ldots, x_n)$ of $n$ variables, then we can take
$n^{2}$ many second derivatives, namely for any choice of
$i$ and $j$:
...
of $x_i$, and then perturbing it in $x_j$ and then comparing
the result of that with what happens if we perturb first $x_j$ and
then $x_i$, with the knowledge that both of these orders lead to
the same final change in the output of $f$.
...
$\mathbf{x}_0$, as we saw in a single variable.
...
(22.4.26), all at the point zero:

$$
(22.4.28)\[\begin{split}\begin{aligned}
f(0,0) & = a, \\
...
$$
(22.4.32)\[\begin{split}\nabla f(x, y) = e^{-x^2-y^2}\begin{pmatrix}1-2x^2 \\ -2xy\end{pmatrix} \; \textrm{and} \; \mathbf{H}f(x, y) = e^{-x^2-y^2}\begin{pmatrix} 4x^3 - 6x & 4x^2y - 2y \\ 4x^2y-2y &4xy^2-2x\end{pmatrix}.\end{split}
$$
...
![../_images/output_multivariable-calculus_bf678a_63_0.svg](../_images/output_multivariable-calculus_bf678a_63_0.svg)

This forms the basis for Newton’s Algorithm discussed in
Section 12.3, where we perform numerical optimization iteratively
...
vector $\boldsymbol{\beta}$, and we want to take the product
function $f(\mathbf{x}) = \boldsymbol{\beta}^\top\mathbf{x}$, and
...
$$
(22.4.34)\[\begin{split}\frac{df}{d\mathbf{x}} = \begin{bmatrix}
\frac{df}{dx_1} \\
\vdots \\
\frac{df}{dx_n}
...
$\beta_1$, note that everything is zero but the first term, which
is just $x_1$ multiplied by $\beta_1$, so we obtain that
...
$$
(22.4.38)\[\begin{split}\frac{df}{d\mathbf{x}} = \begin{bmatrix}
\frac{df}{dx_1} \\
\vdots \\
\frac{df}{dx_n}
...
\vdots \\
\beta_n
\end{bmatrix} = \boldsymbol{\beta}.\end{split}
$$
...
- Second, The final results are much cleaner than the intermediate
  ...
  this case, note that $\frac{d}{dx}(bx) = b$ and
  $\frac{d}{d\mathbf{x}} (\boldsymbol{\beta}^\top\mathbf{x}) = \boldsymbol{\beta}$
  are both similar.
...
harder. Suppose that we have a column vector $\mathbf{x}$, and a
square matrix $A$ and we want to compute
...
To compute our derivative, we need to understand for every $k$,
...
For a term like $\frac{dx_i}{dx_k}$, it is not hard to see that
this is one when $i=k$ and zero otherwise. This means that every
term where $i$ and $k$ are different vanish from this sum,
so the only terms that remain in that first sum are the ones where
$i=k$. The same reasoning holds for the second term where we need
$j=k$. This gives
...
fact that $i$ and $j$ are different is immaterial to this
...
$a_{ki} + a_{ik}$ is the $k, i$-th component of
$\mathbf{A} + \mathbf{A}^\top$. This gives
...
$\mathbf{A} + \mathbf{A}^\top$ by the vector $\mathbf{x}$,
so we see that
...
Thus, we see that the $k$-th entry of the desired derivative from
(22.4.39) is just the $k$-th entry of the vector on
...
Equivalently $\frac{d}{dx}(ax^2) = 2ax = (a+a)x$. Again, we get a
...
of $1 \times 1$ matrices, in which case the matrix product is just
...
Let’s try this out. Suppose that $\mathbf{X}$ is a
$n \times m$ matrix, $\mathbf{U}$ is an $n \times r$
and $\mathbf{V}$ is an $r \times m$. Let’s try to compute
...
imagine what this would be for $1\times1$ matrices. In that case,
...
$\mathbf{X}$ is $n \times m$, as is
$\mathbf{U}\mathbf{V}$, so the matrix
$2(\mathbf{X} - \mathbf{U}\mathbf{V})$ is $n \times m$. On
the other hand $\mathbf{U}$ is $n \times r$, and we cannot
multiply a $n \times m$ and a $n \times r$ matrix since the
dimensions do not match!

We want to get $\frac{d}{d\mathbf{V}}$, which is the same shape as
$\mathbf{V}$, which is $r \times m$. So somehow we need to
take a $n \times m$ matrix and a $n \times r$ matrix,
multiply them together (perhaps with some transposes) to get a
$r \times m$. We can do this by multiplying $U^\top$ by
$(\mathbf{X} - \mathbf{U}\mathbf{V})$. Thus, we can guess the
solution to (22.4.49) is
...
Recalling that all entries of $\mathbf{X}$ and $\mathbf{U}$
are constants as far as $\frac{d}{dv_{ab}}$ is concerned, we may
...
$\frac{dv_{kj}}{dv_{ab}}$ is only non-zero if the $k=a$ and
$j=b$. If either of those conditions do not hold, the term in the
...
An important subtlety here is that the requirement that $k=a$ does
not occur inside the inner sum since that $k$ is a dummy variable
...
We want this to look like the $a, b$ element of a matrix so we can
...
indices on $u_{ia}$. If we notice that
$u_{ia} = [\mathbf{U}^\top]_{ai}$, we can then write
...
and thus we may write the solution to (22.4.49)
...
indeed there are such rules and (Petersen and Pedersen, 2008)
...
1. Given a column vector $\boldsymbol{\beta}$, compute the
   derivatives of both
   $f(\mathbf{x}) = \boldsymbol{\beta}^\top\mathbf{x}$ and
   $g(\mathbf{x}) = \mathbf{x}^\top\boldsymbol{\beta}$. Why do you
   get the same answer?
2. Let $\mathbf{v}$ be an $n$ dimension vector. What is
   $\frac{\partial}{\partial\mathbf{v}}\|\mathbf{v}\|_2$?
3. Let $L(x, y) = \log(e^x + e^y)$. Compute the gradient. What is
   the sum of the components of the gradient?
4. Let $f(x, y) = x^2y + xy^2$. Show that the only critical point
   is $(0,0)$. By considering $f(x, x)$, determine if
   $(0,0)$ is a maximum, minimum, or neither.
5. Suppose that we are minimizing a function
   $f(\mathbf{x}) = g(\mathbf{x}) + h(\mathbf{x})$. How can we
   geometrically interpret the condition of $\nabla f = 0$ in
   terms of $g$ and $h$?
