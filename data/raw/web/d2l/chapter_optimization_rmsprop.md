---
source_url: "https://d2l.ai/chapter_optimization/rmsprop.html"
title: "12.8. RMSProp"
chapter: "12"
section_number: "12.8"
date: null
...
One of the key issues in Section 12.7 is that the learning
...
Tieleman and Hinton (2012) proposed the RMSProp algorithm as a
...
$\mathbf{g}_t$ into a state vector
$\mathbf{s}_t = \mathbf{s}_{t-1} + \mathbf{g}_t^2$. As a result
...
$$
(12.8.1)\[\begin{split}\begin{aligned}
    \mathbf{s}_t & \leftarrow \gamma \mathbf{s}_{t-1} + (1 - \gamma) \mathbf{g}_t^2, \\
...
$$
(12.8.2)\[\begin{split}\begin{aligned}
\mathbf{s}_t & = (1 - \gamma) \mathbf{g}_t^2 + \gamma \mathbf{s}_{t-1} \\
& = (1 - \gamma) \left(\mathbf{g}_t^2 + \gamma \mathbf{g}_{t-1}^2 + \gamma^2 \mathbf{g}_{t-2} + \ldots, \right).
\end{aligned}\end{split}
$$

As before in Section 12.6 we use
$1 + \gamma + \gamma^2 + \ldots, = \frac{1}{1-\gamma}$. Hence the
...
![../_images/output_rmsprop_251805_3_0.svg](../_images/output_rmsprop_251805_3_0.svg)

## 12.8.2. Implementation from Scratch

As before we use the quadratic function
$f(\mathbf{x})=0.1x_1^2+2x_2^2$ to observe the trajectory of
RMSProp. Recall that in Section 12.7, when we used Adagrad
...
quickly. Since $\eta$ is controlled separately this does not
happen with RMSProp.
...
![../_images/output_rmsprop_251805_15_1.svg](../_images/output_rmsprop_251805_15_1.svg)
...
$\gamma$ to 0.9. That is, $\mathbf{s}$ aggregates on average
over the past $1/(1-\gamma) = 10$ observations of the square
gradient.
...
![../_images/output_rmsprop_251805_39_1.svg](../_images/output_rmsprop_251805_39_1.svg)
...
algorithm named `rmsprop`, assigning $\gamma$ to the parameter
`gamma1`.
...
![../_images/output_rmsprop_251805_51_1.svg](../_images/output_rmsprop_251805_51_1.svg)
...
- The learning rate needs to be scheduled by the experimenter in
  practice.
- The coefficient $\gamma$ determines how long the history is
...
2. Rotate the optimization problem to minimize
   $f(\mathbf{x}) = 0.1 (x_1 + x_2)^2 + 2 (x_1 - x_2)^2$. What
...
4. Would you want to adjust $\gamma$ as optimization progresses?
