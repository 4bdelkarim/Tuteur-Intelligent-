---
source_url: "https://d2l.ai/chapter_appendix-mathematics-for-deep-learning/integral-calculus.html"
title: "22.5. Integral Calculus"
chapter: "22"
section_number: "22.5"
date: null
extractor: "d2l"
...
Suppose that we have a function $f(x)$. For simplicity, let’s
assume that $f(x)$ is non-negative (never takes a value less than
...
![../_images/output_integral-calculus_06e2a4_3_0.svg](../_images/output_integral-calculus_06e2a4_3_0.svg)

In most cases, this area will be infinite or undefined (consider the
area under $f(x) = x^{2}$), so people will often talk about the
area between a pair of ends, say $a$ and $b$.
...
![../_images/output_integral-calculus_06e2a4_15_0.svg](../_images/output_integral-calculus_06e2a4_15_0.svg)

We will denote this area by the integral symbol below:

$$
(22.5.1)\[\textrm{Area}(\mathcal{A}) = \int_a^b f(x) \;dx.
$$

The inner variable is a dummy variable, much like the index of a sum in
a $\sum$, and so this can be equivalently written with any inner
value we like:

$$
(22.5.2)\[\int_a^b f(x) \;dx = \int_a^b f(z) \;dz.
$$
...
$a$ and $b$ and chopping it into $N$ vertical slices.
If $N$ is large, we can approximate the area of each slice by a
...
![../_images/output_integral-calculus_06e2a4_27_0.svg](../_images/output_integral-calculus_06e2a4_27_0.svg)
...
$$
(22.5.4)\[\int_a^b \frac{x}{1+x^{2}} \;dx.
$$
...
$$
(22.5.5)\[F(x) = \int_0^x f(y) dy.
$$

This function measures the area between $0$ and $x$
depending on how we change $x$. Notice that this is everything we
need since

$$
(22.5.6)\[\int_a^b f(x) \;dx = F(b) - F(a).
$$
...
point as indicated in Fig. 22.5.1.

![../_images/sub-area.svg](../_images/sub-area.svg)

Fig. 22.5.1 Visualizing why we may reduce the problem of computing the area under
...
$$
(22.5.7)\[F(x+\epsilon) - F(x) = \int_x^{x+\epsilon} f(y) \; dy.
$$
...
rectangular area with height the value of $f(x)$ and the base
width $\epsilon$. Indeed, one can show that as
$\epsilon \rightarrow 0$ this approximation becomes better and
better. Thus we can conclude:

$$
(22.5.8)\[F(x+\epsilon) - F(x) \approx \epsilon f(x).
$$

However, we can now notice: this is exactly the pattern we expect if we
were computing the derivative of $F$! Thus we see the following
rather surprising fact:

$$
(22.5.9)\[\frac{dF}{dx}(x) = f(x).
$$
...
$$
(22.5.10)\[\frac{d}{dx}\int_0^x  f(y) \; dy = f(x).
$$
...
tell us exactly what $F(x)$ is. Indeed $F(x) + C$ for any
$C$ has the same derivative. This is a fact-of-life in the theory
...
$$
(22.5.11)\[\int_a^b f(x) \; dx = (F(b) + C) - (F(a) + C) = F(b) - F(a).
$$
...
Section 22.3.2. For instance, we know that the
derivative of $x^{n}$ is $nx^{n-1}$. Thus, we can say using
the fundamental theorem (22.5.10) that

$$
(22.5.12)\[\int_0^{x} ny^{n-1} \; dy = x^n - 0^n = x^n.
$$

Similarly, we know that the derivative of $e^{x}$ is itself, so
that means

$$
(22.5.13)\[\int_0^{x} e^{x} \; dx = e^{x} - e^{0} = e^x - 1.
$$
...
$$
(22.5.14)\[F(x) = \int_0^x f(y) \; dy.
$$

Let’s suppose that we want to know how this function looks when we
compose it with another to obtain $F(u(x))$. By the chain rule, we
know

$$
(22.5.15)\[\frac{d}{dx}F(u(x)) = \frac{dF}{du}(u(x))\cdot \frac{du}{dx}.
$$

We can turn this into a statement about integration by using the
fundamental theorem (22.5.10) as above. This gives

$$
(22.5.16)\[F(u(x)) - F(u(0)) = \int_0^x \frac{dF}{du}(u(y))\cdot \frac{du}{dy} \;dy.
$$

Recalling that $F$ is itself an integral gives that the left hand
side may be rewritten to be

$$
(22.5.17)\[\int_{u(0)}^{u(x)} f(y) \; dy = \int_0^x \frac{dF}{du}(u(y))\cdot \frac{du}{dy} \;dy.
$$

Similarly, recalling that $F$ is an integral allows us to
recognize that $\frac{dF}{dx} = f$ using the fundamental theorem
(22.5.10), and thus we may conclude

$$
(22.5.18)\[\int_{u(0)}^{u(x)} f(y) \; dy = \int_0^x f(u(y))\cdot \frac{du}{dy} \;dy.
$$

This is the *change of variables* formula.

For a more intuitive derivation, consider what happens when we take an
integral of $f(u(x))$ between $x$ and $x+\epsilon$.
For a small $\epsilon$, this integral is approximately
$\epsilon f(u(x))$, the area of the associated rectangle. Now,
let’s compare this with the integral of $f(y)$ from $u(x)$
to $u(x+\epsilon)$. We know that
$u(x+\epsilon) \approx u(x) + \epsilon \frac{du}{dx}(x)$, so the
area of this rectangle is approximately
$\epsilon \frac{du}{dx}(x)f(u(x))$. Thus, to make the area of
these two rectangles to agree, we need to multiply the first one by
$\frac{du}{dx}(x)$ as is illustrated in
Fig. 22.5.2.

![../_images/rect-trans.svg](../_images/rect-trans.svg)

Fig. 22.5.2 Visualizing the transformation of a single thin rectangle under the
change of variables.

This tells us that

$$
(22.5.19)\[\int_x^{x+\epsilon} f(u(y))\frac{du}{dy}(y)\;dy = \int_{u(x)}^{u(x+\epsilon)} f(y) \; dy.
$$

This is the change of variables formula expressed for a single small
rectangle.

If $u(x)$ and $f(x)$ are properly chosen, this can allow for
the computation of incredibly complex integrals. For instance, if we
even chose $f(y) = 1$ and $u(x) = e^{-x^{2}}$ (which means
$\frac{du}{dx}(x) = -2xe^{-x^{2}}$), this can show for instance
that

$$
(22.5.20)\[e^{-1} - 1 = \int_{e^{-0}}^{e^{-1}} 1 \; dy = -2\int_0^{1} ye^{-y^2}\;dy,
$$

and thus by rearranging that

$$
(22.5.21)\[\int_0^{1} ye^{-y^2}\; dy = \frac{1-e^{-1}}{2}.
$$
...
$$
(22.5.22)\[\int_{e^{-0}}^{e^{-1}} 1 \; dy = e^{-1} -1 < 0,
$$
...
two ways. First, if we consider a function $f(x)$ which is
sometimes less than zero, then the area will also be negative. So for
instance

$$
(22.5.23)\[\int_0^{1} (-1)\;dx = -1.
$$
...
$x$-axis to get the integral of a negative number, or flipping
over the $y$-axis to get an integral in the wrong order) will
...
$$
(22.5.25)\[\int_0^{-1} (-1)\;dx =  1.
$$

If this discussion sounds familiar, it is! In
Section 22.1 we discussed how the
...
suppose that we have a function of two variables, like $f(x, y)$
and we want to know the volume under $f$ when $x$ ranges
...
![../_images/output_integral-calculus_06e2a4_39_0.svg](../_images/output_integral-calculus_06e2a4_39_0.svg)

We write this as

$$
(22.5.26)\[\int_{[a, b]\times[c, d]} f(x, y)\;dx\;dy.
$$

Suppose that we wish to compute this integral. My claim is that we can
do this by iteratively computing first the integral in $x$ and
then shifting to the integral in $y$, that is to say

$$
(22.5.27)\[\int_{[a, b]\times[c, d]} f(x, y)\;dx\;dy = \int_c^{d} \left(\int_a^{b} f(x, y) \;dx\right) \; dy.
$$

Let’s see why this is.

Consider the figure above where we have split the function into
$\epsilon \times \epsilon$ squares which we will index with
integer coordinates $i, j$. In this case, our integral is
approximately

$$
(22.5.28)\[\sum_{i, j} \epsilon^{2} f(\epsilon i, \epsilon j).
$$
...
values. This is illustrated in Fig. 22.5.3. In particular,
we can say that

$$
(22.5.29)\[\sum _ {j} \epsilon \left(\sum_{i} \epsilon f(\epsilon i, \epsilon j)\right).
$$

![../_images/sum-order.svg](../_images/sum-order.svg)

Fig. 22.5.3 Illustrating how to decompose a sum over many squares as a sum over
...
$$
(22.5.30)\[G(\epsilon j) = \int _a^{b} f(x, \epsilon j) \; dx.
$$

Finally, notice that if we combine these two expressions we get

$$
(22.5.31)\[\sum _ {j} \epsilon G(\epsilon j) \approx \int _ {c}^{d} G(y) \; dy = \int _ {[a, b]\times[c, d]} f(x, y)\;dx\;dy.
$$

Thus putting it all together, we have that

$$
(22.5.32)\[\int _ {[a, b]\times[c, d]} f(x, y)\;dx\;dy = \int _ c^{d} \left(\int _ a^{b} f(x, y) \;dx\right) \; dy.
$$
...
$f(x, y) = xy(x^2-y^2)/(x^2+y^2)^3$ over the rectangle
$[0,2]\times[0,1]$).

Note that the choice to do the integral in $x$ first, and then the
integral in $y$ was arbitrary. We could have equally well chosen
to do $y$ first and then $x$ to see

$$
(22.5.33)\[\int _ {[a, b]\times[c, d]} f(x, y)\;dx\;dy = \int _ a^{b} \left(\int _ c^{d} f(x, y) \;dy\right) \; dx.
$$

Often times, we will condense down to vector notation, and say that for
$U = [a, b]\times [c, d]$ this is

$$
(22.5.34)\[\int _ U f(\mathbf{x})\;d\mathbf{x}.
$$

## 22.5.6. Change of Variables in Multiple Integrals

As with single variables in (22.5.18), the ability to
...
take this to be $\phi : \mathbb{R}^n \rightarrow \mathbb{R}^n$,
that is any function which takes in $n$ real variables and returns
another $n$. To keep the expressions clean, we will assume that
$\phi$ is *injective* which is to say it never folds over itself
($\phi(\mathbf{x}) = \phi(\mathbf{y}) \implies \mathbf{x} = \mathbf{y}$).

In this case, we can say that

$$
(22.5.35)\[\int _ {\phi(U)} f(\mathbf{x})\;d\mathbf{x} = \int _ {U} f(\phi(\mathbf{x})) \left|\det(D\phi(\mathbf{x}))\right|\;d\mathbf{x}.
$$

where $D\phi$ is the *Jacobian* of $\phi$, which is the
matrix of partial derivatives of
$\boldsymbol{\phi} = (\phi_1(x_1, \ldots, x_n), \ldots, \phi_n(x_1, \ldots, x_n))$,

$$
(22.5.36)\[\begin{split}D\boldsymbol{\phi} = \begin{bmatrix}
...
chain rule (22.5.18), except we have replaced the term
$\frac{du}{dx}(x)$ with
$\left|\det(D\phi(\mathbf{x}))\right|$. Let’s see how we can to
interpret this term. Recall that the $\frac{du}{dx}(x)$ term
existed to say how much we stretched our $x$-axis by applying
$u$. The same process in higher dimensions is to determine how
much we stretch the area (or volume, or hyper-volume) of a little square
(or little *hyper-cube*) by applying $\boldsymbol{\phi}$. If
$\boldsymbol{\phi}$ was the multiplication by a matrix, then we
know how the determinant already gives the answer.

With some work, one can show that the *Jacobian* provides the best
approximation to a multivariable function $\boldsymbol{\phi}$ at a
...
$$
(22.5.37)\[\int _ {-\infty}^{\infty} \int _ {-\infty}^{\infty} e^{-x^{2}-y^{2}} \;dx\;dy.
$$
...
$\boldsymbol{\phi}(r, \theta) = (r \cos(\theta), r\sin(\theta))$
(which is to say that $x = r \cos(\theta)$,
$y = r \sin(\theta)$), then we can apply the change of variable
formula to see that this is the same thing as

$$
(22.5.38)\[\int _ 0^\infty \int_0 ^ {2\pi} e^{-r^{2}} \left|\det(D\mathbf{\phi}(\mathbf{x}))\right|\;d\theta\;dr,
$$

where

$$
(22.5.39)\[\begin{split}\left|\det(D\mathbf{\phi}(\mathbf{x}))\right| = \left|\det\begin{bmatrix}
\cos(\theta) & -r\sin(\theta) \\
\sin(\theta) & r\cos(\theta)
\end{bmatrix}\right| = r(\cos^{2}(\theta) + \sin^{2}(\theta)) = r.\end{split}
$$

Thus, the integral is

$$
(22.5.40)\[\int _ 0^\infty \int _ 0 ^ {2\pi} re^{-r^{2}} \;d\theta\;dr = 2\pi\int _ 0^\infty re^{-r^{2}} \;dr = \pi,
$$

where the final equality follows by the same computation that we used in
section Section 22.5.3.

We will meet this integral again when we study continuous random
variables in Section 22.6.
...
4. Use the change of variables formula to compute
   $\int_0^2\int_0^1xy(x^2-y^2)/(x^2+y^2)^3\;dy\;dx$ and
   $\int_0^1\int_0^2f(x, y) = xy(x^2-y^2)/(x^2+y^2)^3\;dx\;dy$ to
