---
source_url: "https://d2l.ai/chapter_recurrent-neural-networks/sequence.html"
title: "9.1. Working with Sequences"
chapter: "9"
section_number: "9.1"
date: null
extractor: "d2l"
...
single feature vector $\mathbf{x} \in \mathbb{R}^d$. The main
...
of feature vectors $\mathbf{x}_1, \dots, \mathbf{x}_T$, where each
feature vector $\mathbf{x}_t$ is indexed by a time step
$t \in \mathbb{Z}^+$ lying in $\mathbb{R}^d$.
...
its own sequence of words, and each having its own length $T_i$;
...
$P(X)$. While we still assume that entire sequences (e.g., entire
...
We sometimes wish to predict a fixed target $y$ given sequentially
...
target ($y_1, \ldots, y_T$) given a fixed input (e.g., image
...
(Fig. 9.1.1). At each *time step*
$t \in \mathbb{Z}^+$, we observe the price, $x_t$, of the
index at that time.
...
$$
(9.1.1)\[P(x_t \mid x_{t-1}, \ldots, x_1)
$$
...
$$
(9.1.2)\[\mathbb{E}[(x_t \mid x_{t-1}, \ldots, x_1)],
$$

would be to apply a linear regression model (recall
Section 3.1). Such models that regress the value
...
number of inputs, $x_{t-1}, \ldots, x_1$ varies, depending on
$t$. In other words, the number of inputs increases with the
...
where the object of interest is $P(x_t \mid x_{t-1}, \ldots, x_1)$
or some statistic(s) of this distribution.

A few strategies recur frequently. First of all, we might believe that
although long sequences $x_{t-1}, \ldots, x_1$ are available, it
...
some window of length $\tau$ and only use
$x_{t-1}, \ldots, x_{t-\tau}$ observations. The immediate benefit
is that now the number of arguments is always the same, at least for
$t > \tau$. This allows us to train any linear model or deep
network that requires fixed-length vectors as inputs. Second, we might
develop models that maintain some summary $h_t$ of the past
observations (see Fig. 9.1.2) and at the same time
update $h_t$ in addition to the prediction $\hat{x}_t$. This
leads to models that estimate not only $x_t$ with
$\hat{x}_t = P(x_t \mid h_{t})$ but also updates of the form
$h_t = g(h_{t-1}, x_{t-1})$. Since $h_t$ is never observed,
these models are also called *latent autoregressive models*.

![../_images/sequence-model.svg](../_images/sequence-model.svg)
...
of $x_t$ might change, the dynamics according to which each
...
$p(x_1, \ldots, x_T)$ into the product of conditional densities in
a left-to-right fashion by applying the chain rule of probability:

$$
(9.1.3)\[P(x_1, \ldots, x_T) = P(x_1) \prod_{t=2}^T P(x_t \mid x_{t-1}, \ldots, x_1).
$$
...
we condition only on the $\tau$ previous time steps, i.e.,
$x_{t-1}, \ldots, x_{t-\tau}$, rather than the entire sequence
history $x_{t-1}, \ldots, x_1$. Whenever we can throw away the
history beyond the previous $\tau$ steps without any loss in
...
past, given the recent history*. When $\tau = 1$, we say that the
data is characterized by a *first-order Markov model*, and when
$\tau = k$, we say that the data is characterized by a
$k^{\textrm{th}}$-order Markov model. For when the first-order
Markov condition holds ($\tau = 1$) the factorization of our joint
probability becomes a product of probabilities of each word given the
previous *word*:

$$
(9.1.4)\[P(x_1, \ldots, x_T) = P(x_1) \prod_{t=2}^T P(x_t \mid x_{t-1}).
$$
...
validity depends on a $k^{\textrm{th}}$-order Markov condition.
...
relative frequency estimate of $P(x_t \mid x_{t-1})$. Whenever the
...
sequence $P(x_1, \ldots, x_T)$ as a left-to-right chain of
...
unfolding $P(x_1, \ldots, x_T)$ in reverse order. The result is a
valid factorization:

$$
(9.1.5)\[P(x_1, \ldots, x_T) = P(x_T) \prod_{t=T-1}^1 P(x_t \mid x_{t+1}, \ldots, x_T).
$$
...
probability over steps $1$ through $t$ into one that extends
to word $t+1$ we simply multiply by the conditional probability of
...
the past. Hence, if we change $x_t$, we may be able to influence
what happens for $x_{t+1}$ going forward but not the converse.
That is, if we change $x_t$, the distribution over past events
will not change. In some contexts, this makes it easier to predict
$P(x_{t+1} \mid x_t)$ than to predict $P(x_t \mid x_{t+1})$.
For instance, in some cases, we can find
$x_{t+1} = f(x_t) + \epsilon$ for some additive noise
$\epsilon$, whereas the converse is not true
(Hoyer et al., 2009). This is great news, since it is
typically the forward direction that we are interested in estimating.
The book by Peters *et al.* (2017) contains more on
...
![../_images/output_sequence_ce248f_18_0.svg](../_images/output_sequence_ce248f_18_0.svg)

To begin, we try a model that acts as if the data satisfied a
$\tau^{\textrm{th}}$-order Markov condition, and thus predicts
$x_t$ using only the past $\tau$ observations. Thus for each
time step we have an example with label $y = x_t$ and features
$\mathbf{x}_t = [x_{t-\tau}, \ldots, x_{t-1}]$. The astute reader
might have noticed that this results in $1000-\tau$ examples,
since we lack sufficient history for $y_1, \ldots, y_\tau$. While
we could pad the first $\tau$ sequences with zeros, to keep things
simple, we drop them for now. The resulting dataset contains
$T - \tau$ examples, where each input to the model has sequence
length $\tau$. We create a data iterator on the first 600
...
![../_images/output_sequence_ce248f_48_0.svg](../_images/output_sequence_ce248f_48_0.svg)
...
![../_images/output_sequence_ce248f_63_0.svg](../_images/output_sequence_ce248f_63_0.svg)
...
inputs, having seen only up to $x_{604}$. We can address this
...
$$
(9.1.6)\[\begin{split}\begin{aligned}
\hat{x}_{605} &= f(x_{601}, x_{602}, x_{603}, x_{604}), \\
\hat{x}_{606} &= f(x_{602}, x_{603}, x_{604}, \hat{x}_{605}), \\
\hat{x}_{607} &= f(x_{603}, x_{604}, \hat{x}_{605}, \hat{x}_{606}),\\
\hat{x}_{608} &= f(x_{604}, \hat{x}_{605}, \hat{x}_{606}, \hat{x}_{607}),\\
\hat{x}_{609} &= f(\hat{x}_{605}, \hat{x}_{606}, \hat{x}_{607}, \hat{x}_{608}),\\
&\vdots\end{aligned}\end{split}
$$

Generally, for an observed sequence $x_1, \ldots, x_t$, its
predicted output $\hat{x}_{t+k}$ at time step $t+k$ is
called the $k$*-step-ahead prediction*. Since we have observed
up to $x_{604}$, its $k$-step-ahead prediction is
$\hat{x}_{604+k}$. In other words, we will have to keep on using
...
![../_images/output_sequence_ce248f_78_0.svg](../_images/output_sequence_ce248f_78_0.svg)
...
that after step 1 we have some error $\epsilon_1 = \bar\epsilon$.
Now the *input* for step 2 is perturbed by $\epsilon_1$, hence we
suffer some error in the order of
$\epsilon_2 = \bar\epsilon + c \epsilon_1$ for some constant
$c$, and so on. The predictions can diverge rapidly from the true
...
Let’s take a closer look at the difficulties in $k$-step-ahead
...
![../_images/output_sequence_ce248f_93_0.svg](../_images/output_sequence_ce248f_93_0.svg)
...
sequence up to time step $t$, its predicted output at time step
$t+k$ is the $k$*-step-ahead prediction*. As we predict
further in time by increasing $k$, the errors accumulate and the
...
1. Improve the model in the experiment of this section.
   ...
   2. How many past observations would you need if there was no noise?
      Hint: you can write $\sin$ and $\cos$ as a
