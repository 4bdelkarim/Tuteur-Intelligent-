---
source_url: "https://d2l.ai/chapter_convolutional-modern/densenet.html"
title: "8.7. Densely Connected Networks (DenseNet)"
chapter: "8"
section_number: "8.7"
date: null
extractor: "d2l"
...
(Huang et al., 2017). DenseNet is characterized by
...
Recall the Taylor expansion for functions. At the point $x = 0$ it
can be written as
...
![../_images/densenet-block.svg](../_images/densenet-block.svg)

Fig. 8.7.1 The main difference between ResNet (left) and DenseNet (right) in
cross-layer connections: use of addition and use of concatenation.

As shown in Fig. 8.7.1, the key difference between
ResNet and DenseNet is that in the latter case outputs are
*concatenated* (denoted by $[,]$) rather than added. As a result,
we perform a mapping from $\mathbf{x}$ to its values after
...
![../_images/densenet.svg](../_images/densenet.svg)

Fig. 8.7.2 Dense connections in DenseNet. Note how the dimensionality increases
with depth.
...
Section 8.6). First, we implement this convolution block
structure.
...
channels, we will get an output with $3 + 10 + 10=23$ channels.
...
number of channels by using a $1\times 1$ convolution. Moreover,
...
to 4, consistent with the ResNet-18 model in Section 8.6.
...
![../_images/output_densenet_e82156_123_0.svg](../_images/output_densenet_e82156_123_0.svg)
...
3. One problem for which DenseNet has been criticized is its high memory
   ...
   1. Is this really the case? Try to change the input shape to
      $224\times 224$ to compare the actual GPU memory consumption
      empirically.
...
5. Design an MLP-based model by applying the DenseNet idea. Apply it to
   the housing price prediction task in Section 5.7.
