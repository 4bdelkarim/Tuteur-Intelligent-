---
source_url: "https://d2l.ai/chapter_computational-performance/multiple-gpus.html"
title: "13.5. Training on Multiple GPUs"
chapter: "13"
section_number: "13.5"
date: null
extractor: "d2l"
...
Section 13.3. We also showed in Section 6.7 how
...
Section 13.6. We assume that you are familiar with
minibatch stochastic gradient descent algorithms such as the ones
described in Section 12.5.

## 13.5.1. Splitting the Problem

Let’s start with a simple computer vision problem and a slightly archaic
...
partition. See e.g., Mirhoseini *et al.* (2017) for a best
...
Fig. 13.5.1 (taken from
Krizhevsky *et al.* (2012)) illustrates this design,
...
![../_images/alexnet-original.svg](../_images/alexnet-original.svg)

Fig. 13.5.1 Model parallelism in the original AlexNet design due to limited GPU
memory.
...
![../_images/splitting.svg](../_images/splitting.svg)

Fig. 13.5.2 Parallelization on multiple GPUs. From left to right: original
...
Assume that there are $k$ GPUs on a machine. Given the model to be
...
synchronized. As an example, Fig. 13.5.3 illustrates
training with data parallelism when $k=2$.

![../_images/data-parallel.svg](../_images/data-parallel.svg)

Fig. 13.5.3 Calculation of minibatch stochastic gradient descent using data
...
- In any iteration of training, given a random minibatch, we split the
  examples in the batch into $k$ portions and distribute them
...
- The local gradients of each of the $k$ GPUs are aggregated to
...
Note that in practice we *increase* the minibatch size $k$-fold
when training on $k$ GPUs such that each GPU has the same amount
...
normalization in Section 8.5 needs to be adjusted, e.g.,
...
We use LeNet as introduced in Section 7.6 (with slight
modifications). We define it from scratch to illustrate parameter
exchange and synchronization in detail.
...
For efficient multi-GPU training we need two basic operations. First we
need to have the ability to distribute a list of parameters to multiple
devices and to attach gradients (`get_params`). Without parameters it
...
Since we did not perform any computation yet, the gradient with regard
...
Let’s test this by creating vectors with different values on different
devices and aggregate them.
...
We need a simple utility function to distribute a minibatch evenly
...
learning framework to try it out on a $4 \times 5$ matrix.
...
For later reuse we define a `split_batch` function that splits both
data and labels.
...
discussed, `allreduce` and `split_and_load`, to synchronize the data
...
is processed using the `train_batch` function to deal with multiple
GPUs. For convenience (and conciseness of code) we compute the accuracy
on a single GPU, though this is *inefficient* since the other GPUs are
idle.
...
Let’s see how well this works on a single GPU. We first use a batch size
of 256 and a learning rate of 0.2.

```python
train(num_gpus=1, batch_size=256, lr=0.2)
```

![../_images/output_multiple-gpus_f17d18_93_1.svg](../_images/output_multiple-gpus_f17d18_93_1.svg)

By keeping the batch size and learning rate unchanged and increasing the
...
![../_images/output_multiple-gpus_f17d18_102_1.svg](../_images/output_multiple-gpus_f17d18_102_1.svg)

## 13.5.7. Summary

- There are multiple ways to split deep network training over multiple
...
- Data parallel training is straightforward. However, it increases the
  effective minibatch size to be efficient.
- In data parallelism, data is split across multiple GPUs, where each
  GPU executes its own forward and backward operation and subsequently
  gradients are aggregated and results are broadcast back to the GPUs.
- We may use slightly increased learning rates for larger minibatches.

## 13.5.8. Exercises

1. When training on $k$ GPUs, change the minibatch size from
   $b$ to $k \cdot b$, i.e., scale it up by the number of
   GPUs.
2. Compare accuracy for different learning rates. How does it scale with
   the number of GPUs?
3. Implement a more efficient `allreduce` function that aggregates
   different parameters on different GPUs? Why is it more efficient?
