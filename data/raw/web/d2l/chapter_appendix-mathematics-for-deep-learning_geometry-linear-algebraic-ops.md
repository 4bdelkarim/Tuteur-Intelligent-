---
source_url: "https://d2l.ai/chapter_appendix-mathematics-for-deep-learning/geometry-linear-algebraic-ops.html"
title: "22.1. Geometry and Linear Algebraic Operations"
chapter: "22"
section_number: "22.1"
date: null
extractor: "d2l"
...
In Section 2.3, we encountered the basics of linear
...
machine learning more broadly. While Section 2.3
...
$$
(22.1.1)\[\begin{split}\mathbf{x} = \begin{bmatrix}1\\7\\0\\1\end{bmatrix},\end{split}
$$

or

$$
(22.1.2)\[\mathbf{x}^\top = \begin{bmatrix}1 & 7 & 0 & 1\end{bmatrix}.
$$
...
Section 2.3, though a single vector’s default
...
This can be seen in Fig. 22.1.1.

![../_images/grid-points.svg](../_images/grid-points.svg)

Fig. 22.1.1 An illustration of visualizing vectors as points in the plane. The
first component of the vector gives the
$\mathit{x}$-coordinate, the second component gives the
$\mathit{y}$-coordinate. Higher dimensions are analogous,
...
$\mathbf{v} = [3,2]^\top$ as the location $3$ units to the
right and $2$ units up from the origin, we can also think of it as
the direction itself to take $3$ steps to the right and $2$
steps up. In this way, we consider all the vectors in figure
Fig. 22.1.2 the same.

![../_images/par-vec.svg](../_images/par-vec.svg)

Fig. 22.1.2 Any vector can be visualized as an arrow in the plane. In this case,
...
as is seen in Fig. 22.1.3.

![../_images/vec-add.svg](../_images/vec-add.svg)

Fig. 22.1.3 We can visualize vector addition by first following one vector, and
then another.

Vector subtraction has a similar interpretation. By considering the
identity that $\mathbf{u} = \mathbf{v} + (\mathbf{u}-\mathbf{v})$,
we see that the vector $\mathbf{u}-\mathbf{v}$ is the direction
that takes us from the point $\mathbf{v}$ to the point
$\mathbf{u}$.

## 22.1.2. Dot Products and Angles

As we saw in Section 2.3, if we take two column
vectors $\mathbf{u}$ and $\mathbf{v}$, we can form their dot
product by computing:

$$
(22.1.3)\[\mathbf{u}^\top\mathbf{v} = \sum_i u_i\cdot v_i.
$$

Because (22.1.3) is symmetric, we will mirror the notation
of classical multiplication and write

$$
(22.1.4)\[\mathbf{u}\cdot\mathbf{v} = \mathbf{u}^\top\mathbf{v} = \mathbf{v}^\top\mathbf{u},
$$

to highlight the fact that exchanging the order of the vectors will
yield the same answer.

The dot product (22.1.3) also admits a geometric
interpretation: it is closely related to the angle between two vectors.
Consider the angle shown in Fig. 22.1.4.

![../_images/vec-angle.svg](../_images/vec-angle.svg)

Fig. 22.1.4 Between any two vectors in the plane there is a well defined angle
$\theta$. We will see this angle is intimately tied to the dot
product.

To start, let’s consider two specific vectors:

$$
(22.1.5)\[\mathbf{v} = (r,0) \; \textrm{and} \; \mathbf{w} = (s\cos(\theta), s \sin(\theta)).
$$

The vector $\mathbf{v}$ is length $r$ and runs parallel to
the $x$-axis, and the vector $\mathbf{w}$ is of length
$s$ and at angle $\theta$ with the $x$-axis. If we
compute the dot product of these two vectors, we see that

$$
(22.1.6)\[\mathbf{v}\cdot\mathbf{w} = rs\cos(\theta) = \|\mathbf{v}\|\|\mathbf{w}\|\cos(\theta).
$$
...
$$
(22.1.7)\[\theta = \arccos\left(\frac{\mathbf{v}\cdot\mathbf{w}}{\|\mathbf{v}\|\|\mathbf{w}\|}\right).
$$
...
consider writing $\|\mathbf{v} - \mathbf{w}\|^2$ in two ways: one
...
vectors $\mathbf{v}$ and $\mathbf{w}$, the angle between the
two vectors is

$$
(22.1.8)\[\theta = \arccos\left(\frac{\mathbf{v}\cdot\mathbf{w}}{\|\mathbf{v}\|\|\mathbf{w}\|}\right).
$$
...
$$
(22.1.9)\[\cos(\theta) = \frac{\mathbf{v}\cdot\mathbf{w}}{\|\mathbf{v}\|\|\mathbf{w}\|}.
$$

The cosine takes a maximum value of $1$ when the two vectors point
in the same direction, a minimum value of $-1$ when they point in
opposite directions, and a value of $0$ when the two vectors are
orthogonal. Note that if the components of high-dimensional vectors are
sampled randomly with mean $0$, their cosine will nearly always be
close to $0$.
...
$\mathbf{w}=[2,1]^\top$. We want to know, “what are the points
$\mathbf{v}$ with $\mathbf{w}\cdot\mathbf{v} = 1$?” By
recalling the connection between dot products and angles above
(22.1.8), we can see that this is equivalent to

$$
(22.1.10)\[\|\mathbf{v}\|\|\mathbf{w}\|\cos(\theta) = 1 \; \iff \; \|\mathbf{v}\|\cos(\theta) = \frac{1}{\|\mathbf{w}\|} = \frac{1}{\sqrt{5}}.
$$

![../_images/proj-vec.svg](../_images/proj-vec.svg)

Fig. 22.1.5 Recalling trigonometry, we see the formula
$\|\mathbf{v}\|\cos(\theta)$ is the length of the projection of
the vector $\mathbf{v}$ onto the direction of
$\mathbf{w}$
...
$\mathbf{v}$ onto the direction of $\mathbf{w}$ is exactly
$1/\|\mathbf{w}\|$, as is shown in Fig. 22.1.5.
The set of all points where this is true is a line at right angles to
the vector $\mathbf{w}$. If we wanted, we could find the equation
for this line and see that it is $2x + y = 1$ or equivalently
$y = 1 - 2x$.

If we now look at what happens when we ask about the set of points with
$\mathbf{w}\cdot\mathbf{v} > 1$ or
$\mathbf{w}\cdot\mathbf{v} < 1$, we can see that these are cases
where the projections are longer or shorter than
$1/\|\mathbf{w}\|$, respectively. Thus, those two inequalities
...
![../_images/space-division.svg](../_images/space-division.svg)

Fig. 22.1.6 If we now consider the inequality version of the expression, we see
...
$\mathbf{w} = [1,2,3]^\top$ and ask about the points in three
dimensions with $\mathbf{w}\cdot\mathbf{v} = 1$, we obtain a plane
at right angles to the given vector $\mathbf{w}$. The two
inequalities again define the two sides of the plane as is shown in
Fig. 22.1.7.

![../_images/space-division-3d.svg](../_images/space-division-3d.svg)
...
Section 4.1, as methods to find hyperplanes that separate
...
Fashion-MNIST dataset (seen in Section 4.2) by just
...
![../_images/output_geometry-linear-algebraic-ops_80b1c3_39_0.svg](../_images/output_geometry-linear-algebraic-ops_80b1c3_39_0.svg)
...
![../_images/output_geometry-linear-algebraic-ops_80b1c3_51_0.svg](../_images/output_geometry-linear-algebraic-ops_80b1c3_51_0.svg)
...
Through Section 2.3 and the above discussions, we
...
$$
(22.1.11)\[\begin{split}\mathbf{A} = \begin{bmatrix}
a & b \\ c & d
\end{bmatrix}.\end{split}
$$

If we want to apply this to an arbitrary vector
$\mathbf{v} = [x, y]^\top$, we multiply and see that

$$
(22.1.12)\[\begin{split}\begin{aligned}
...
$$
(22.1.13)\[\begin{split}\mathbf{A} = \begin{bmatrix}
1 & 2 \\
-1 & 3
\end{bmatrix}.\end{split}
$$

If we look at the specific vector $\mathbf{v} = [2, -1]^\top$, we
see this is $2\cdot[1,0]^\top + -1\cdot[0,1]^\top$, and thus we
know that the matrix $A$ will send this to
...
structure must remain as you see in Fig. 22.1.8.

![../_images/grid-transform.svg](../_images/grid-transform.svg)

Fig. 22.1.8 The matrix $\mathbf{A}$ acting on the given basis vectors.
...
$$
(22.1.14)\[\begin{split}\mathbf{B} = \begin{bmatrix}
2 & -1 \\ 4 & -2
\end{bmatrix},\end{split}
$$
...
the result from matrix $\mathbf{A}$ can be “bent back” to the
original grid. The results from matrix $\mathbf{B}$ cannot because
we will never know where the vector $[1,2]^\top$ came from—was it
$[1,1]^\top$ or $[0, -1]^\top$?

While this picture was for a $2\times2$ matrix, nothing prevents
us from taking the lessons learned into higher dimensions. If we take
similar basis vectors like $[1,0, \ldots,0]$ and see where our
...
$$
(22.1.15)\[\begin{split}\mathbf{B} = \begin{bmatrix}
2 & -1 \\ 4 & -2
\end{bmatrix}.\end{split}
$$
...
can. Let’s take $\mathbf{b}_1 = [2,4]^\top$ and
...
$$
(22.1.16)\[a_1\mathbf{b}_1 + a_2\mathbf{b}_2 = -2a_1\mathbf{b}_2 + a_2\mathbf{b}_2 = (a_2-2a_1)\mathbf{b}_2.
$$
...
dependence $\mathbf{b}_1 = -2\cdot\mathbf{b}_2$ captures this. To
make this more symmetrical between the two vectors, we will write this
as

$$
(22.1.17)\[\mathbf{b}_1  + 2\cdot\mathbf{b}_2 = 0.
$$
...
$$
(22.1.18)\[\sum_{i=1}^k a_i\mathbf{v_i} = 0.
$$
...
$$
(22.1.19)\[\begin{split}\mathbf{B} = \begin{bmatrix}
2 & 4 \\ -1 & -2
\end{bmatrix},\end{split}
$$

has $\textrm{rank}(B)=1$, since the two columns are linearly
dependent, but either column by itself is not linearly dependent. For a
more challenging example, we can consider

$$
(22.1.20)\[\begin{split}\mathbf{C} = \begin{bmatrix}
1& 3 & 0 & -1 & 0 \\
-1 & 0 & 1 & 1 & -1 \\
0 & 3 & 1 & 0 & -1 \\
2 & 3 & -1 & -2 & 1
...
and show that $\mathbf{C}$ has rank two since, for instance, the
...
full-rank matrix (i.e., some $\mathbf{A}$ that is
$n \times n$ matrix with rank $n$), we should always be able
to undo it. Consider the matrix

$$
(22.1.21)\[\begin{split}\mathbf{I} = \begin{bmatrix}
1 & 0 & \cdots & 0 \\
0 & 1 & \cdots & 0 \\
...
$$
(22.1.22)\[\mathbf{A}^{-1}\mathbf{A} = \mathbf{A}\mathbf{A}^{-1} =  \mathbf{I}.
$$
...
$$
(22.1.23)\[\begin{split}\mathbf{A} = \begin{bmatrix}
a & b \\
c & d
\end{bmatrix},\end{split}
$$
...
$$
(22.1.24)\[\begin{split} \frac{1}{ad-bc}  \begin{bmatrix}
d & -b \\
-c & a
\end{bmatrix}.\end{split}
$$
...
$$
(22.1.25)\[\mathbf{A}\mathbf{x} = \mathbf{b},
$$

than computing the inverse and multiplying to get

$$
(22.1.26)\[\mathbf{x} = \mathbf{A}^{-1}\mathbf{b}.
$$
...
all $1\textrm{M}^2$ entries—that is $1$ trillion entries!
...
![../_images/grid-transform-filled.svg](../_images/grid-transform-filled.svg)

Fig. 22.1.9 The matrix $\mathbf{A}$ again distorting the grid. This time, I
...
$(0, 1)$ and $(1, 0)$ and thus it has area one. After
$\mathbf{A}$ transforms this square, we see that it becomes a
...
$$
(22.1.27)\[\begin{split}\mathbf{A} = \begin{bmatrix}
1 & 2 \\
-1 & 3
\end{bmatrix},\end{split}
$$
...
parallelogram and obtain that the area is $5$.

In general, if we have a matrix

$$
(22.1.28)\[\begin{split}\mathbf{A} = \begin{bmatrix}
a & b \\
c & d
\end{bmatrix},\end{split}
$$
...
$$
(22.1.29)\[\begin{split}\mathbf{B} = \begin{bmatrix}
2 & 4 \\ -1 & -2
\end{bmatrix}.\end{split}
$$

If we compute the determinant of this matrix, we get
$2\cdot(-2 ) - 4\cdot(-1) = 0$. Given our understanding above,
this makes sense. $\mathbf{B}$ compresses the square from the
...
true: a matrix $A$ is invertible if and only if the determinant is
not equal to zero.
...
$n\times n$ matrices scale $n$-dimensional volumes.

## 22.1.9. Tensors and Common Linear Algebra Operations

In Section 2.3 the concept of tensors was introduced.
...
$$
(22.1.30)\[\mathbf{C} = \mathbf{A}\mathbf{B},
$$

or equivalently

$$
(22.1.31)\[c_{i, j} = \sum_{k} a_{i, k}b_{k, j}.
$$
...
$$
(22.1.32)\[y_{il} = \sum_{jk} x_{ijkl}a_{jk}.
$$
...
Section 2.3, we can create tensors as is shown below.
...
($\mathbf{A}\mathbf{v} = a_{ij}v_j$) and strip out the indices
...
$$
(22.1.34)\[c_{kl} = \sum_{ij} \mathbf{b}_{ijk}\mathbf{a}_{il}v_j.
$$
...
- Linear dependence is a way to tell when a collection of vectors are
  ...
  $3$ vectors living in a $2$-dimensional space). The rank
...
$$
(22.1.35)\[\begin{split}\vec v_1 = \begin{bmatrix}
1 \\ 0 \\ -1 \\ 2
...
2. True or false: $\begin{bmatrix}1 & 2\\0&1\end{bmatrix}$ and
   $\begin{bmatrix}1 & -2\\0&1\end{bmatrix}$ are inverses of one
   another?
3. Suppose that we draw a shape in the plane with area
   $100\textrm{m}^2$. What is the area after transforming the
   figure by the matrix

$$
(22.1.36)\[\begin{split}\begin{bmatrix}
2 & 3\\
1 & 2
\end{bmatrix}.\end{split}
$$
...
5. Suppose that you have a matrix written as
   $A = \begin{bmatrix}c\\d\end{bmatrix}\cdot\begin{bmatrix}a & b\end{bmatrix}$
   for some choice of values $a, b, c$, and $d$. True or
   false: the determinant of such a matrix is always $0$?
6. The vectors $e_1 = \begin{bmatrix}1\\0\end{bmatrix}$ and
   $e_2 = \begin{bmatrix}0\\1\end{bmatrix}$ are orthogonal. What
   is the condition on a matrix $A$ so that $Ae_1$ and
   $Ae_2$ are orthogonal?
7. How can you write $\textrm{tr}(\mathbf{A}^4)$ in Einstein
   notation for an arbitrary matrix $A$?
