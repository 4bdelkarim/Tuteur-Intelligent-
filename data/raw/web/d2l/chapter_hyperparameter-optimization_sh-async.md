---
source_url: "https://d2l.ai/chapter_hyperparameter-optimization/sh-async.html"
title: "19.5. Asynchronous Successive Halving"
chapter: "19"
section_number: "19.5"
date: null
extractor: "d2l"
...
As we have seen in Section 19.3, we can accelerate HPO by
...
$r_{\mathrm{min}}$, we first have to evaluate all
$N = \eta^K$ configurations, before we can promote the
$\frac{1}{\eta}$ of them to the next rung level.
...
Figure Fig. 19.5.1 shows the scheduling of synchronous SH
with $\eta=2$ for four different trials with two workers. We start
...
![../_images/sync_sh.svg](../_images/sync_sh.svg)

Fig. 19.5.1 Synchronous successive halving with two workers.

Asynchronous successive halving (ASHA) (Li et al., 2018) adapts SH to
...
$\eta$ observations on the current rung level. This decision rule
...
configuration with $r = r_{\mathrm{min}}$, i.e the first rung
level.

Fig. 19.5.2 shows the scheduling of the same configurations for
...
Trial-3 performs worse than Trial-0 at rung 0, and $\eta=2$, we
...
![../_images/asha.svg](../_images/asha.svg)
...
to the objective function corresponds to $r_{\mathrm{max}}$.
Moreover, `grace_period` provides $r_{\mathrm{min}}$, and
`reduction_factor` is $\eta$. We can run Syne Tune as before
...
![../_images/output_sh-async_bb0ea6_13_1.svg](../_images/output_sh-async_bb0ea6_13_1.svg)
...
search in Section 19.3. As we have seen for successive
halving in Section 19.4, most of the trials are stopped at 1
or 2 epochs ($r_{\mathrm{min}}$ or
$\eta * r_{\mathrm{min}}$). However, trials do not stop at the
...
![../_images/output_sh-async_bb0ea6_15_1.svg](../_images/output_sh-async_bb0ea6_15_1.svg)
