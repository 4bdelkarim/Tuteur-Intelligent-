---
source_url: https://d2l.ai/chapter_preliminaries/ndarray.html
title: 2.1. Data Manipulation
chapter: '2'
section_number: '2.1'
date: null
extractor: d2l
source_type: web
source: chapter_preliminaries_ndarray
---
...
it, so to start, let’s get our hands dirty with $n$-dimensional
...
called a *matrix*. With $k > 2$ axes, we drop the specialized
names and just refer to the object as a $k^\textrm{th}$-*order
tensor*.
...
$1$. Unless otherwise specified, new tensors are stored in main
...
$n$ and target shape ($h$, $w$), we know that
$w = n/h$. To automatically infer one component of the shape, we
...
specified for a $k^\textrm{th}$-order tensor, it is applied along
...
and $v_i$ are the $i^\textrm{th}$ elements of vectors
...
Since `a` and `b` are $3\times1$ and $1\times2$
